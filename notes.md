...
- [x] **L-doors2 [#698]** — Decyzja 2b cz.2: endless `go_deeper` domyka drzwi na styku segmentów (regresja po #697 — boss miał 0 wolnych drzwi → osierocona zaślepka + drzwi-widmo). ✅ `_attach_endless_segment`: attach_dir z REALNYCH drzwi bossa (zdejmuje zaślepkę), opp_dir musi istnieć w kaflu-wejściu, po scaleniu `_fill_open_doors` na całym grafie. Inwariant: ZERO null / ZERO drzwi-widm / ZERO sierot / symetria. pytest 2/2 + Playwright. — [#698](https://github.com/szmidtpiotr/ai-gm/issues/698)
