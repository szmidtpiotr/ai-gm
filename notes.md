...
- [ ] [#761](https://github.com/szmidtpiotr/ai-gm/issues/761) — Rejestr zmian zasobów/kondycji `state_changes` (bez CASCADE): before→after+delta+cause+tura. Wpięte: HP gracza w walce + zmiana strefy; follow-up: mana/heal/rest/trap/level/kondycje. Endpoint + MCP `query_state_changes`. TDD 5/5 + Playwright 2/2. Commit `0051af9` (review)
- [ ] [#762](https://github.com/szmidtpiotr/ai-gm/issues/762) — Rejestr decyzji silnika `turn_decisions` (bez CASCADE): action_type+route+gate verdict/reason per żywa tura (żywy następca martwego action_log). Intent keyword-only (zero kosztu LLM). Endpoint + MCP `query_turn_decisions`. TDD 4/4 + Playwright 1/1. Commit `0051af9` (review)
