...
- [ ] [#760](https://github.com/szmidtpiotr/ai-gm/issues/760) — Czytniki debug MCP/API: `query_world_snapshots` + `query_llm_calls` (ekspozycja istniejących world_state_snapshots/llm_call_log per kampania, przycięte). Zero nowego zbierania. TDD 4/4 + Playwright 2/2. Commit `0051af9` (review)
- [ ] [#761](https://github.com/szmidtpiotr/ai-gm/issues/761) — Rejestr zmian zasobów/kondycji `state_changes` (bez CASCADE): before→after+delta+cause+tura. Wpięte: HP(walka)+strefa+mana(wszystkie czary)+heal+rest+trap+kondycje(apply/expire); follow-up: level-up. Endpoint + MCP `query_state_changes` + zakładka 📊 Stan. TDD 5/5 + Playwright 2/2. Commity `0051af9`+`4ff4583` (review)
- [ ] [#762](https://github.com/szmidtpiotr/ai-gm/issues/762) — Rejestr decyzji silnika `turn_decisions` (bez CASCADE): action_type+confidence+route+gate verdict/reason per żywa tura (żywy następca martwego action_log). Intent przez `intent_service.parse_intent` (keyword-only, zero kosztu LLM). Endpoint + MCP `query_turn_decisions` + zakładka 🧭 Decyzje. TDD 4/4 + Playwright 1/1. Commity `0051af9`+`4ff4583` (review)
