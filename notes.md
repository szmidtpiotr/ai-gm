...
- [ ] L18 — Playwright: regresja lochu end-to-end (wejście→walka→drzwi→zagadka→boss→endless→wyjście + mapa)
  ...
  - 🟢 **Follow-up balansowy [#732]** (commit `1ac3b58`): dungeon-only healing **sustain drop** — po każdej (nie-boss) wygranej walce w lochu tier-skalowana szansa na drop mikstury leczenia (D1 40% → D5 15%), żeby PŻ solo-bohatera nie topniało do zera między walkami. Dungeon-only z konstrukcji (`roll_dungeon_sustain_drop` gated na `dungeon_run.system==tiles_v2`; walka w świecie bez zmian). Wpięte w loot_pool obu ścieżek zwycięstwa → ujawnia się w istniejącym odbiorze łupu (zero zmian we froncie). 16/16 pytest. Wartości startowe (Numbers Policy) — strojenie po żywej grze. **needs-testing**.
