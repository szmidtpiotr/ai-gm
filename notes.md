...
- [ ] L18 — Playwright: regresja lochu end-to-end (wejście→walka→drzwi→zagadka→boss→endless→wyjście + mapa)
  ...
  - 🟡 **[#733] ease-in pierwszych komnat — WDROŻONE (needs-testing)** (commit `5aaff5a`, design po `/brainstorming`): pierwsze 2 komnaty WALKI głównej ścieżki mają cap liczby+sumy bazowych PŻ wrogów skalowany tierem (`_pick_path_candidate` w `_try_build_path`; D1 komnata1 ≤1 wróg ≤16 PŻ, komnata2 ≤2 ≤28; endless bez ease-in). Bramkowanie #729 ograniczało TYPY, to ogranicza LICZBĘ/sumę PŻ — wymuszona pierwsza walka już nie jest 2-3 wrogów/38 PŻ. 39/39 pytest + 10/10 runów krypta w budżecie. **Do domknięcia L18 brakuje pełnego przejazdu `/playwright-test-report L18` solo lvl1 do bossa.**
