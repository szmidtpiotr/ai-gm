...
> - **L-belka [#719→commit]** — modal/przyciski nawigacji lochu (zagadka/skrzynia/D-pad) `bottom:84px` wpadały pod belkę „Co robisz?" + dolny pasek; podniesione na `calc(136px+safe)`. (commit 65801cc)
> - **L-dodge [#719]** — modal kości pokazywał tylko Twój rzut; trafienie to test OPOZYCYJNY (atak vs unik wroga k20+ZRC). Dodana linia „🛡 Unik wroga: … vs Twój atak N" + werdykt hit/miss (atak broni + czar). Front nazywa `data.dodge_roll`, silnik liczy. (commit cefb46d)
> - **L-boss-loot [#720]** — po pokonaniu bossa brak popupu „co wypadło"; łup bossa przyznawany server-side (poza loot_pool), a `on_boss_tile_cleared` zwracał surowy roll bez labeli. Backend zwraca granted (z `label`); front pokazuje popup-rewelację (reveal-only) przed wyborem głębiej/wyjdź. pytest T4b. (commit 3028497)
...
- [x] L13c — 🎮 KAMIEŃ MILOWY (mid-faza): `/game-smoke-dungeon --engine` po Bloku 4 — silnik+UI na kafelkach testowych (przed treścią L14–L16). Łapie bugi grafu/ruchu/mapy/walki/bossa/endless/śmierci/porzucenia ZANIM włożymy treść. Bez TDD, raport do [SMOKE] FAZA L. Checkpointy zależne od treści (opisy PL, zagadki) = N/D. Zaliczone = przebieg silnika bez P0; werdykt min. GRYWALNY Z ZASTRZEŻENIAMI. — ✅ **ZALICZONE 2026-06-16**: silnik GRYWALNY bez P0; #684 (P0) i #685 (P1) naprawione+zamknięte; wizualna weryfikacja UI (FAZA B: #687 D-pad, #688 mapa, mobile 390px); zacommitowane. Pełny smoke z treścią (nadzbiór, raport [#686](https://github.com/szmidtpiotr/ai-gm/issues/686#issuecomment-4723851487)) potwierdził cały silnik end-to-end.
...
### Blok 6 — Weryfikacja
- [x] L18 — Playwright: regresja lochu end-to-end (wejście→walka→drzwi→zagadka→boss→endless→wyjście + mapa) — ✅ **ZAMKNIĘTE 2026-06-17**: weryfikacja end-to-end osiągnięta przez L13c (engine smoke) + L19 (ręczne przejście Piotra mobile, 14/14 checkpointów) + LB6 (Playwright onboarding); #733/#734 → reklasyfikowane do LB4 (katakumby_mroku), NIE blokery krypta_probna; soft-init LB2 (#736) zweryfikowany.
  - 🧪 **TEST balansu 2026-06-17** (`/playwright-test-report`, solo Wojownik lvl1 w `krypta_probna` D1): werdykt ❌ **BLOKER** — loch „Poz. 1+" za trudny dla samotnego lvl1. Pierwsza walka (kafel 302) = Cień 35PŻ/AC14/+7 + Mroczny Czarodziej 14PŻ/+6 vs bohater 12PŻ/AC11; w rundzie 3 bohater 2/12 „Poważnie Ranny", wrogowie nietknięci → pewna śmierć. Root cause: skala oderwana od poziomu bohatera (L5) + pula kafli krypty bez krzywej (miesza 1×szkielet z shadow_stalker/3×ghoul/undead_champion) + boss D1=Lisz 90PŻ one-shotuje. Raport: [#731](https://github.com/szmidtpiotr/ai-gm/issues/731). Bugi: **#729** (balans, P0/P1), **#730** (nakładka „Tura wroga" zawisa, recover tylko reload — pokrewne #700). Pełna regresja e2e (drzwi→zagadka→boss→endless→mapa) NIEdokończona — walka blokowała.
  - 🟢 **FIX + RETEST 2026-06-17** (`/playwright-test-report`, ten sam scenariusz): werdykt ✅. #729 — `enemy_pool` bramkuje losowanie kafli walki + tier-softening bossa (`BOSS_TIER_FACTOR`, Lisz D1 90→40 PŻ); pierwsza walka = pojedynczy Szkielet 10 PŻ zamiast Cień+Czarodziej. #730 — `_reconcileCombatTurnUI` zawsze synchronizuje nakładkę „Tura wroga" z backendem + poll 2 s. Retest wykrył wtórny objaw #730: modal rzudu kością `#dice-overlay` zawisał (brak callbacku osiadania kostki 3D) → naprawiony bezpiecznikiem 4 s w `playCombatDiceRoll`. Commity `432322a` + `15fd46b`. Raport: [#731 comment](https://github.com/szmidtpiotr/ai-gm/issues/731#issuecomment-4731939970). **#729/#730/#731 ZAMKNIĘTE** (zweryfikowane retestem).
  - 🟢 **[#732] dungeon-only sustain drop — ZWERYFIKOWANE + ZAMKNIĘTE** (commits `1ac3b58` + fix `626fad8`): po każdej (nie-boss) wygranej walce w lochu tier-skalowana szansa (D1 40%→D5 15%) na drop mikstury leczenia prosto do plecaka. Dungeon-only z konstrukcji (`grant_dungeon_victory_sustain` gated na `dungeon_run.system==tiles_v2`; świat→brak). Retest `/playwright-test-report` wykrył, że pierwsze podejście (loot_pool) NIE przyznawało konsumpcyjnego → przepięte na bezpośrednie przyznanie po stronie serwera + toast. Mikstura widoczna w plecaku („Mała mikstura leczenia"). 16/16 pytest. Raport: [#732 comment](https://github.com/szmidtpiotr/ai-gm/issues/732#issuecomment-4732508946).
  - 🟡 **[#733] ease-in pierwszych komnat — WDROŻONE, NIEWYSTARCZAJĄCE** (commit `5aaff5a`): cap liczby+sumy bazowych PŻ wrogów na pierwszych 2 komnatach walki głównej ścieżki (D1 komnata1 ≤1 wróg ≤16 PŻ, komnata2 ≤2 ≤28; endless off). 39/39 pytest + 10/10 runów w budżecie. **Finalny przejazd `/playwright-test-report` 2026-06-17 ([raport #733](https://github.com/szmidtpiotr/ai-gm/issues/733#issuecomment-4732883886)): ❌ bohater ZGINĄŁ w 1. walce mimo ease-in.** Skład ograniczony poprawnie (1 Duch 16 PŻ), ale: (a) wróg wygrał inicjatywę + cios 6 obr. (połowa PŻ), (b) **brak użycia mikstury w walce → #734**. Diagnoza: pojedynczy 16-PŻ wróg vs 12-PŻ bohater to wciąż rzut monetą bez mid-fight heal.
  - 🔴 **[#734] BLOKER L18:** pasek Akcja w walce = tylko Cofnij/Zapasy — brak „Użyj mikstury". Sustain #732 + mikstury bezużyteczne w trakcie starcia. Spec zakłada „mikstury na groźne walki" (`game_mechanics.md`). Priorytet do domknięcia L18.
  - 🟠 **[#733] strojenie:** obniżyć cap PŻ komnaty 1 D1 16→~10 (najsłabszy wróg) lub zmiękczyć pierwszy cios. **L18 NIE do zamknięcia, dopóki #734 (+ strojenie #733) niezrobione.**
- [x] L20a — Portrety wrogów/NPC: persystencja (migracja image_url na game_config_enemies+npcs, update_enemy/update_npc, portret BASE_PROMPT, batch --entity) — naprawia zepsuty zapis istniejącego modala (TDD; przed L19) — [#692](https://github.com/szmidtpiotr/ai-gm/issues/692) ✅ commit d4ff5d6 (5/5 pytest + 3/3 Playwright)
- [x] L20b — Portrety wrogów/NPC: display u gracza (modal startu walki + miniatura w chipie inicjatywy, fallback emoji; ożywia [NPC_INTERACTION]) + USUNIĘCIE bloku „Pozycje sprite'ów na kafelku" z edytora kafla (TDD) — [#724](https://github.com/szmidtpiotr/ai-gm/issues/724) ✅ 3/3 pytest + 4/4 Playwright
- [x] L20c — Portrety nieumarłych krypty: pilot 5 → akceptacja Piotra → batch (bez TDD) — [#725](https://github.com/szmidtpiotr/ai-gm/issues/725) ✅ 2026-06-17: 8/8 nieumarłych krypty (skeleton, zombie, ghoul, ghost, wraith, lich, vampire_master, undead_champion) mają portrety. Model **Juggernaut-XL v9 / 768×1024 / 25 kroków** (FLUX schnell 8-krok dawał artefakty — akceptacja jakości po regeneracji). Numbers Policy CZĘŚĆ AJ zaktualizowane.
- [x] L19 — 🎮 KAMIEŃ MILOWY: pełny playtest lochu skillem `/game-smoke-dungeon` (na treści krypta po L16: 2 cykle endless, śmierć z checkpointem, porzucenie, mapa, mobile; 14 checkpointów; raport do [SMOKE] FAZA L; bez TDD). Pierwszy kandydat na loch GRYWALNY. — ✅ **ZALICZONE 2026-06-17** (ręczne przejście Piotra na mobile): loch GRYWALNY end-to-end. Bugi z przejścia naprawione: #719 (belka zakrywała przyciski lochu + modal kości pokazuje rzut wroga na unik), #720 (popup łupu bossa). Drobny tuning: dwell modalu kości 2800→3600 ms.
  - SMOKE PEŁNY 2026-06-16 (`/game-smoke-dungeon` krypta_probna + `/playwright-test-report` wizual): **werdykt GRYWALNY (po fixach)**. 14/14 checkpointów ✅ (DB + UI). Znalezione i naprawione defekty: **#716 P1** resolve-tile 500 na WSZYSTKICH zagadkach (riddle string key vs dict; fix `94cb187`, TDD L6 24/24); **#687** dungeon-nav nie chował się po wejściu na walkę, nachodził na pasek walki 390px (fix `820fd78`); **#699** porzucenie z modala-wznowienia nie czyściło runu + cooldown na złym bohaterze (fix `6a9d0a9`); **#688** modal mapy ✕ — zweryfikowany OK. Test-only: **#717** (test_l8 extend RED na fiksturze test_cat po #698, produkt OK). Zweryfikowane+zamknięte wcześniejsze: #684/#685/#673/#675/#676/#677/#689/#698. Raport: [#686 comment](https://github.com/szmidtpiotr/ai-gm/issues/686#issuecomment-4723851487).
...
- [x] LB1 — Mechanika: **odpoczynek na wyczyszczonym kaflu** (kafel „bezpieczny" po pokonaniu wrogów → akcja rest na węźle, NIE nowy kafel). **Backend (4504fff):** migracja `game_dungeons.rest_heal_pct`(20)/`rest_charges`(2); `_dungeon_rest_policy` + `_action_rest` gated na `cleared`, `_apply_heal_to_character` realnie leczy, licznik `rest_used`, onboarding 100%/unlimited vs głębszy 20%/ładunki, `onboarding_note`; 26/26 pytest. **Frontend (#735):** pill „🕯 Pokonano wrogów — możesz odpocząć" + przycisk „🕯 Odpocznij" w `dungeon-nav__actions` (zielony), widoczne gdy `enemies>0 && cleared`; `_dungeonResolveTile('rest')` → POST `/dungeons/resolve-tile {action:"rest"}`, refresh HP + toast `hp_after` + toast `onboarding_note`; obsługa `blocked`/`no_charges`. Bump `?v=lb1-735-rest`. DOM/JS zweryfikowane na DEV (elementy istnieją, wired, brak nowych błędów konsoli). → `[TASK] LB1`
- [x] LB2 — **soft-init komnata 1**: bohater zawsze pierwszy w PIERWSZEJ walce runu (neutralizuje śmierć-na-inicjatywie z testu L18). Kod inicjatywy, dungeon-scoped. TDD. **Backend (41926ad):** `initiate_combat` nowy param `_dungeon_first_combat`; `move_through_door` śledzi `combats_started` w run dict i przekazuje flagę na 1. walkę; 4/4 pytest. → `[#736](https://github.com/szmidtpiotr/ai-gm/issues/736)`
- [x] LB3 — **`krypta_probna` = ONBOARDING** (re-spec config/seed): entry + 2 komnaty walki + boss = `tile_count=4`, `min_level=1`, `rest_heal_pct=100`, `rest_charges=0` (unlimited). Cel clear ~78-85% solo lvl1. Walidacja sim+Sandbox+Playwright (LB6). **Seed v2 (override Piotra 2026-06-17):** **boss = `undead_champion` (~20 PŻ przy D1)** zamiast nowego `krypta_opiekun`, `tile_count=4` zamiast 3 → realnie 2 komnaty walki przed bossem. Boss tile `Komnata Strażnika` (id=305) przepięty na undead_champion; legacy `krypta_opiekun` dezaktywowany (is_active=0); UPDATE krypta_probna tile_count=4/min_level=1/rest_heal_pct=100/rest_charges=0/boss_enemy=undead_champion/boss_tile_id=305; 5/5 pytest niezmiennika config + graf 40/40 GREEN. Seed v1 (3441ff1): tile_count=3/boss~18HP/krypta_opiekun — nadpisane. → `[#737](https://github.com/szmidtpiotr/ai-gm/issues/737)`
- [x] LB4 — **nowy głębszy loch `katakumby_mroku`** (config/seed): obecna treść 5 walk + Lisz 40 PŻ, absolutna skala, `min_level=3`, `rest_heal_pct=20`. Tu przenoszą się i mają sens: **#733 (ease-in), #732 (sustain), #734 (mid-fight heal)**. Cel clear ~60-70% @ min_level. **Seed (9f9e52e):** `katakumby_mroku` INSERT — tile_count=7 (entry+5 combat+boss), min_level=3, rest_heal_pct=20, rest_charges=2, boss=`lich` (tile id=24 „Tron Lisza", HP@D1≈41 PŻ), tile_category=krypta; 6/6 pytest GREEN. → `[#738](https://github.com/szmidtpiotr/ai-gm/issues/738)`
- [ ] LB5 — (przyszłość MP, osobny ticket): skaluj LICZBĘ wrogów rozmiarem drużyny (siłę zostaw tierem). Nie teraz.
- [x] LB6 — Walidacja końcowa: Playwright przejazd onboarding solo lvl1 — /playwright-test-report 2026-06-17, raport: https://github.com/szmidtpiotr/ai-gm/issues/738#issuecomment-4734223535. **Werdykt ⚠️:** krypta_probna (onboarding) ✅ działa (nawigacja, walka, rest 3→9/9 HP = 100%), ALE katakumby_mroku min_level=3 NIE jest egzekwowany — lvl1 wchodzi bez blokady → **#739**. **#739 NAPRAWIONE (84c48e3):** backend 403 `level_too_low` + picker lockuje lochy powyżej poziomu (🔒 Wymagany Poz. N); `_get_hero_level` liczony z `xp_lifetime_earned`. 4 testy pytest GREEN, zweryfikowane live na DEV.
...
- [x] [#777](https://github.com/szmidtpiotr/ai-gm/issues/777) — Instrumentacja narracyjna: game_events dla quest_complete/xp_grant/location_new/item_grant/gold_grant; _record_turn_decision_safe wpięta do streaming path (turn_decisions było 0 globalnie). TDD 8/8 + Playwright 3/3. Commit 3639fdd. needs-testing
- [x] [#781](https://github.com/szmidtpiotr/ai-gm/issues/781) — Zakładka 🗓 Zdarzenia w monitorze kampanii (companion #777): endpoint GET /campaigns/{id}/game-events + tab events w campaigns.js (ikony EV map, filtry per typ, ekstrakcja szczegółów). TDD 5/5 + Playwright 3/3 + weryfikacja wizualna DEV. Commit fd6b45d. needs-testing
- [x] [#780](https://github.com/szmidtpiotr/ai-gm/issues/780) — Atak z zaskoczenia + bramka intencji po przewadze: D1 bramka [Atak/Zastraszenie/Wycofaj] po sukcesie Stealth (build_advantage_gate w combat_service, emisja w skill-test/resolve, renderAdvantageGate w app.js); D2 first_hit_auto (auto-trafienie zaskoczonego); burst — sneak die pali się przy otwarciu ze skradania (cel zaskoczony), nie tylko gdy gracz hidden; D3 skalowanie kości sneak (L1-4 1d6 / L5-8 2d6 / L9+ 3d6); uczciwy komunikat korekty (combat_correction_message — koniec kłamstwa o „nieosiągalności"); system_prompt non-lethal intent → SKILL_TEST intimidation bez COMBAT_START. Supersedes #778. TDD 14/14 + Playwright 1/1 + weryfikacja wizualna bramki DEV. needs-testing
...
- [x] [#761](https://github.com/szmidtpiotr/ai-gm/issues/761) — Rejestr zmian zasobów/kondycji `state_changes` (bez CASCADE): before→after+delta+cause+tura. Wpięte: HP(walka)+strefa+mana(wszystkie czary)+heal+rest+trap+kondycje(apply/expire); follow-up: level-up. Endpoint + MCP `query_state_changes` + zakładka 📊 Stan. TDD 5/5 + Playwright 2/2. Commity `0051af9`+`4ff4583`. ✅ closed
- [x] [#762](https://github.com/szmidtpiotr/ai-gm/issues/762) — Rejestr decyzji silnika `turn_decisions` (bez CASCADE): action_type+confidence+route+gate verdict/reason per żywa tura (żywy następca martwego action_log). Intent przez `intent_service.parse_intent` (keyword-only, zero kosztu LLM). Endpoint + MCP `query_turn_decisions` + zakładka 🧭 Decyzje. TDD 4/4 + Playwright 1/1. Commity `0051af9`+`4ff4583`. ✅ closed
- [x] [#754](https://github.com/szmidtpiotr/ai-gm/issues/754) — Strukturalny rejestr rzutów kostką: tabela `dice_rolls` (bez CASCADE — przeżywa wyjście z lochu), helper `record_dice_roll` wpięty w atak gracza/wroga, obrażenia, unik, blok, save, test skilla, loot, gold; endpoint `GET /api/campaigns/{id}/dice-rolls`; narzędzie MCP `query_dice_rolls`; zakładka 🎲 Rzuty w monitorze kampanii. TDD 5/5 + Playwright 2/2. Bug po drodze: loot/gold tylko na ścieżce AoE — dopięte na pojedynczym ubiciu (`97e95fb`). Zweryfikowane na żywym lochu #99796. Commity `b35391d` + `97e95fb`. ✅ closed
- [x] [#770](https://github.com/szmidtpiotr/ai-gm/issues/770) — Ekwipunek: podział sekcji wg używalności (can_use) zamiast item_type. `_invIsLore` → `_invIsUsable`; narracyjne/misc/quest → dół "PRZEDMIOTY NIEUŻYWALNE"; broń/zbroja/consumable → góra PLECAK. TDD 9/9 + Playwright 2/2. Commit `aeb9ad7`. needs-testing
- [x] [#756](https://github.com/szmidtpiotr/ai-gm/issues/756) — Quest dedup po celu: Jaccard word-similarity (≥0.65) w persist_quest_to_character_quests odrzuca duplikaty mimo innego tytułu; blok QUESTY wstrzyknięty do promptu LLM; reguła anty-duplikat w system_prompt.txt. TDD 6/6 + Playwright 2/2. Commit `0af61e2`. ✅ przetestowane /playwright-test-report 2026-06-19 — 1 quest w UI/API/DB (było 4), raport: https://github.com/szmidtpiotr/ai-gm/issues/756#issuecomment-4749916282
- [x] [#771](https://github.com/szmidtpiotr/ai-gm/issues/771) — Konsumable on-use effect builder: kolumna effect_json w game_config_consumables, dispatcher damage_enemy (zadaje obrażenia pierwszemu żywemu wrogowi w active_combat), apply_condition z target=enemy/self, backfill 14 konsumabli (holy_water, alchemist_fire, scroll_fireball, potions…), _CONSUMABLE_EFFECT_TYPES w admin content.js + forge.js. TDD 8/8 + Playwright 3/3. Commit `c095229`. needs-testing
- [x] [#775](https://github.com/szmidtpiotr/ai-gm/issues/775) — Monetary grant_item safety-net: `_monetary_item_gold()` w turns.py wykrywa mieszek/sakiewka+monet/zapłat i konwertuje na grant_gold zamiast tworzyć martwy item; _bag_gold accumulator (explicit Grant Gold N zawsze wygrywa); ZAKAZ grant_item dla pieniędzy w system_prompt.txt. TDD 9/9 + Playwright 3/3. Commit `444d85b`. needs-testing
...
- [x] U4b — Playtest LLM 15 tur × 2 tryby skillem /game-smoke (ruch po hexach, NPC, quest, walka, sklep, odpoczynek+XP, beaty w Gotowej) — defekty P0/P1/P2; werdykt "brak P0" z U4 NIE jest jeszcze potwierdzony — [#512](https://github.com/szmidtpiotr/ai-gm/issues/512) [#513](https://github.com/szmidtpiotr/ai-gm/issues/513) **OBA NIEGRYWALNY — P0: #515 (scene_enemies), P1: #518 #519 #520 #521 #522**
  - SMOKE 2026-06-16 (przeglądarka/Playwright, nowa-kampania #99763): werdykt **GRYWALNY** (bez potwierdzonych P0/P1). Działa: start+HUD, ruch/zmiana scen, walka (start→ciosy→koniec), zegar (09→10 na walce), ucieczka, spójność. **#712 wycofane jako invalid** — walka wręcz gracza używa przeciwstawnego uniku (`compute_player_attack_dodge_outcome`, testy test_phase8_combat.py), nie płaskiego `total≥DEF`; total 18 legalnie uniknięte przez Wilka (dex+2). Checkpointy quest/sklep/odpoczynek/gate/lokacje-NPC z bazy = N/D (dziki start bez pobliskiej treści; LLM nie doprowadził do osady w 8 turach — do decyzji czy defekt). Raport: [#713](https://github.com/szmidtpiotr/ai-gm/issues/713)
  - FIX #714 — randomizacja lokacji startowej — **przetestowane 2026-06-16** (Playwright, 3 kampanie: Torven/Veldra/Aldric). Werdykt ✅ DZIAŁA: `current_location_id` wskazuje prawdziwą lokację z `game_locations` (np. Cieszowice/town), LLM generuje narrację osadzoną w konkretnym miejscu. Raport: [#714#issuecomment-4720845573](https://github.com/szmidtpiotr/ai-gm/issues/714#issuecomment-4720845573)
...
> - **Combat-UI** (kampania 99767): #669 (clear sceny 3D PRZED overlay — kod 5736-5747 + brak ducha na żywo), #641 (etykiety „Krytyczny sukces/porażka" ujednolicone w app.js), #599 (mobile 390px: baner nie zasłania narracji, kolumny ZWARCIE/DYSTANS widoczne), #597 (rozbicie rzutu „🎲 19 +0 Inteligencja +1 Ranga = 20"). **Wszystkie ✅ — zamknięte.**
> - **❌ #535 (Gate ATTACK regex łapie negację) — NADAL WYSTĘPUJE.** Tura „Nic nie atakuję. Chowam laskę..." → backend `combat_start_tag_injected` (enemy=wolf) → combat 144, mimo pokojowej narracji LLM bez `[COMBAT_START]`. Regex łapie „atakuję" w „nic nie atakuję". Ticket **otwarty**, wymaga poprawki guard'a negacji przed wstrzyknięciem combat_start. (Bogus combat 144 sprzątnięty.)
> - **Combat-gate siblings #534/#520/#596 — wszystkie ZOSTAWIONE OTWARTE (nie potwierdzone naprawienie).** Ta sama dziurawa ścieżka `_validate_combat_start_target` co #535. Test #596 „Atakuję goblina!" (brak goblina w scenie) → walka wystartowała (combat 145), ale LLM sam wprowadził goblina → nierozstrzygnięte. #534/#520 nieodtworzone deterministycznie. **Rekomendacja: jeden spójny re-fix gate'a (negacja + obecność w lokacji + przyjazny NPC + injekcja tagu w obie strony) + deterministyczne testy, razem z #535.** (Bogus combaty 144/145 sprzątnięte.)
> - **Inne:** #715 (✅ zamknięte — 0 lokacji `Start N` w bazie, nowa kampania 99767 nie utworzyła placeholderów), #537 (✅ zamknięte — XP hooks żyją: 45 grantów w `character_xp_grants`, świeże `exploration.location_new` dziś; obserwacja poboczna: kampania 99767 nie dostała „pierwsza wizyta" XP — możliwa interakcja z #715, do rzutu okiem). **❌ #701 — fix NIESKUTECZNY (otwarte):** w `combat_reconcile.js` gałąź `watchdog_resync` to martwy kod — guard `!out.reason || out.reason==="fetch_in_flight"` nigdy nie łapie defaultu `"no_combat"`, więc DESYNC nadal zwraca `no_combat`.

> **RE-FIX combat-gate #535/#596/#534/#520 (2026-06-16, ai-gm-tdd + /playwright-test-report, commity `9f1d2e4` + `02bf8ec`):** Jeden spójny guard w `backend/app/api/turns.py` (`_player_combat_intent` + `_validate_combat_start_target` + `_ensure_combat_start_tag`). (a) negacja „nie/nic nie atakuję" wyłapywana przed injekcją; (b) cel musi być obecny w scenie/narracji — koniec fallbacku na cały katalog `game_config_enemies`; (c) przyjazny questodawca/NPC odrzucany przed katalogiem; (d) odwrotna injekcja przy bojowej narracji bez tagu, z pre-walidacją. **Druga warstwa błędu wykryta na żywo i naprawiona (`02bf8ec`):** walidacja jest teraz świadoma źródła tagu — tag wstrzyknięty przez backend na słowa gracza wymaga obecności + agresji (samo wymienienie nazwy w *zaprzeczeniu* „nie ma żadnego goblina" już nie startuje walki), tag od Mistrza Gry zaufany. Testy: `backend/tests/test_combat_gate_unified.py` (23) + zaktualizowany `test_combat_start_validation.py` = **30/30 GREEN**. Live (demo, camp 99767): „Nic nie atakuję" → `combat_gm_tag_absent`, brak injekcji, brak walki ✅; „Atakuję smoka" (nieobecny) → `combat_gm_tag_rejected_hf7` `combat_target_not_present`, `active_combat`=0 ✅; „Atakuję goblina" → Mistrz Gry świadomie wprowadza zasadzkę (`combat_gm_tag_started`, bez injekcji) = obronne. **#535/#596 zweryfikowane wizualnie → ZAMKNIĘTE.** #534/#520 pokryte testem deterministycznym, nieodtworzone wizualnie → **otwarte z `needs-testing`**. Raport: [#535 comment](https://github.com/szmidtpiotr/ai-gm/issues/535).
...
- [stale-fixed → do zamknięcia po weryfikacji wizualnej] #566 (sendCombatNarration→stream), #520/#534/#535/#549/#553 (U30/U5-6/U28-29/HF-6/7/9/11) — kod naprawiony, czekają na wizualną weryfikację Piotra.
- [x] [#643](https://github.com/szmidtpiotr/ai-gm/issues/643) — surowy `<strong>` w narracji travel arrival: `_executeTravelFromPill` używała dosłownego HTML zamiast markdown; fix: `**${destLabel}**` (commit 1f1fa11). Cinematic path (_wmExecuteTravel, innerHTML) działała poprawnie. /playwright-test-report 2026-06-16, hex travel live verified ✅
- [x] [#518](https://github.com/szmidtpiotr/ai-gm/issues/518) — current_hex nie zmieniał się: zweryfikowane live na DEV, {0,0}→{0,1}→{1,1} po hex travel i narrative turn. Closed 2026-06-16.
