...
- [x] LB3 — **`krypta_probna` = ONBOARDING** (re-spec config/seed): 2 komnaty walki (1 wróg/komnata), boss ~18 PŻ, `tile_count=3`, `min_level=1`, `rest_heal_pct=100`. Cel clear ~78-85% solo lvl1. Walidacja sim+Sandbox+Playwright. **Seed (3441ff1):** nowy enemy `krypta_opiekun` (hp_base=40→18 PŻ at D1), nowy boss tile `Komnata Strażnika` (id=305), UPDATE krypta_probna tile_count=3/rest_heal_pct=100/rest_charges=0/boss_tile_id=305; ease-in gwarantuje ≤1 wróg w komnacie środkowej (cycle=1, COUNT_CAP=1); 5/5 pytest GREEN. → `[#737](https://github.com/szmidtpiotr/ai-gm/issues/737)`
