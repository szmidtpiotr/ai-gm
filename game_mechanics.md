...
- **Startowy zestaw maga (L1, 4× tier 1):** `fire_bolt` (atak) + `minor_heal` (heal) + **`ward_of_iron`** (obrona) + `detect_magic` (utility). Pełna tożsamość od startu. ✅ **Wdrożone B8 ([#655](https://github.com/szmidtpiotr/ai-gm/issues/655), 2026-06-15).** **Decyzja D-obronna (Piotr 2026-06-15):** obronny = `ward_of_iron` (tier 1), NIE `mage_armor` (tier 2) — zachowanie zasady „4× tier 1" + spójność z bramką nauki L1 z B7 (`max_tier=ceil(lvl/2)=1`); dziś oba placeholdery (prawdziwa absorpcja=B10), więc tiebreaker = tier. Migracja `v2-spells-faza-b-b8-starter-backfill` dosiewa zestaw istniejącym magom NIE-destrukcyjnie (stare czary zostają).
