...
##### S17-EXT — Follow-up zapasów gated za rang (decyzja Piotra 2026-06-15) ✅ CZĘŚĆ A (SOLO) ZROBIONE [#622] · Część B ⛔ FAZA 5

> **Wdrożone Część A (solo, 2026-06-15):** `combat_service.resolve_wrestling` — po udanych zapasach (`SUCCESS`/`CRITICAL_SUCCESS`) gracza z `wrestling` rank ≥ 3 silnik dolicza SŁABSZY darmowy cios w tej samej turze: rzut bronią gracza (`resolve_sheet_weapon`, fallback 1d6/STR), obrażenia **÷2** (floor, min 1), zalogowany jako oddzielny event `wrestling_followup` (raz — inline, bez pętli/rekurencji). Rank 1–2 = czysta kontrola jak dotąd (baza nietknięta = balans chroniony). Reuse EKONOMII `extra_action` (S12): wrestling i tak woła `advance_turn` RAZ → follow-up nie pochłania kolejnej akcji (turę przejmuje wróg po jednym ciosie). BEZ nat-20-double (follow-up nie używa rzutu ataku), BEZ ponownego wyzwalania zapasów. Śmierć wroga od follow-upu domyka `advance_turn` (victory, `_all_enemies_dead`). Frontend gracza: karta „💪 Dodatkowy cios — wróg traci N HP (połowa obrażeń)" w `app.js?v=622` (event w filtrze feedu + gałąź renderująca). Zero zmian schematu DB. 6/6 pytest real-engine (`test_issue622_*`: rank3 sukces/krytyk → cios ÷2; rank2 brak; rank3 porażka brak; min 1; turę raz) + 13/13 backward-compat #612 + 1/1 Playwright GREEN. Liczby (próg ranku 3, ÷2) = wartości startowe (Numbers Policy → tuning po B5/B13). **Część B (MP przewaga sojusznika dla schwytanego wroga): ⛔ ODŁOŻONE do FAZY 5** — wymaga towarzyszy/MP + systemu reakcji (dziś nieistniejące).
...
- #754 — Strukturalny rejestr rzutów kostką (debug): tabela `dice_rolls` (świeża, BEZ FK CASCADE → rzuty przeżywają wyjście z lochu, w odróżnieniu od `combat_turns`). Helper `dice_log_service.record_dice_roll()` (best-effort, nigdy nie wywraca tury) wpięty w: atak gracza/wroga + obrażenia NdX (`combat_service`), unik/blok/save (`_record_reaction_rolls`), test skilla (`skill_service.resolve_skill_test` — jeden punkt dla wszystkich ścieżek), loot+gold (combat death). Każdy wiersz: surowy d20/kości, rozbicie modyfikatorów, DC, outcome, powiązanie z turą/walką. Wystawione: `GET /api/campaigns/{id}/dice-rolls` + narzędzie MCP `query_dice_rolls` + zakładka 🎲 Rzuty w monitorze kampanii. Martwy `action_log`/`process_v2_turn` świadomie pominięty (czysty schemat). Bug po drodze: loot/gold rejestrowane tylko na ścieżce śmierci AoE — dopięte też na pojedynczym ubiciu (`97e95fb`). Zweryfikowane na żywym lochu #99796 (walka ze szkieletem — wszystkie typy rzutów widoczne). TDD 5/5 + Playwright 2/2. Commity `b35391d` + `97e95fb`. ✅ closed
- #760/#761/#762 — instrumentacja debug na żywo (companion #754), żeby sesje były debugowalne przez MCP/API bez replayu (tanio tokenowo). #760: czytniki MCP `query_world_snapshots`+`query_llm_calls` (odczyt istniejących world_state_snapshots/llm_call_log). #761: tabela `state_changes` (bez CASCADE) + `state_log_service` wpięte w HP gracza (walka) i strefę — before→after+delta+cause; MCP `query_state_changes`. #762: tabela `turn_decisions` (bez CASCADE) + `decision_log_service`; żywa ścieżka `create_turn` zapisuje intent (keyword stub, zero LLM), route, gate verdict+reason per tura — żywy następca martwego action_log; MCP `query_turn_decisions`. Endpointy GET pod `/api/campaigns/{id}/{state-changes,turn-decisions,world-snapshots}` + `/api/admin/campaigns/{id}/llm-calls`. TDD 13/13 + Playwright 5/5. Commit `0051af9`. ✅ needs-testing
...
**Decyzja 2b — Wszystkie narysowane drzwi muszą być przejściem (Piotr, 2026-06-16; rozszerza Decyzję 2; task [#697]).**
Obraz kafla jest jeden per typ i pokazuje wszystkie kanoniczne drzwi z `doors_json`. Dziś generator (`_build_tile_graph`) inicjalizuje `doors_open = {d: None for d in _doors(tile)}` i wypełnia tylko połączone, więc namalowane-ale-niepołączone drzwi (`doors_open.X = null`) mylą gracza (widzi drzwi na obrazku, d-pad ich nie pokazuje). **Reguła:** generator dobiera kafle z bazy tak, by KAŻDE narysowane drzwi każdego kafla prowadziło do sąsiada; prawdopodobnie wymaga to kafli „zaślepek" o 1 drzwiach (cap tiles) per kategoria, żeby zamknąć ślepe odnogi bez kaskady nowych dziur. Jeśli grafu z zerem ślepych drzwi nie da się zbudować (kolizje siatki / brak pasującego kafla) → **przegeneruj ścieżkę** (retry); po wyczerpaniu prób fallback do najlepszego grafu, by nie zablokować wejścia. Realizacja 2-cz.: (A) treść — 1-drzwiowe kafle krypty z obrazkami FLUX (jak L14–L15) — ❌ DO ZROBIENIA, (B) generator (L2, TDD) — ✅ ZROBIONE (#697, 2026-06-16): `_fill_open_doors` domyka każde otwarte drzwi — najpierw weld (zlanie sąsiadujących otwartych drzwi bez nowego kafla), potem cap-fill 1-drzwiową zaślepką (zero kaskady). `draw_tile_graph` zwraca graf o ZERO ślepych drzwi gdy kategoria ma komplet zaślepek N/S/E/W; w przeciwnym razie fallback do najlepszego grafu (entry nigdy zablokowane) + log ostrzegawczy. Weryfikacja: `GET /api/admin/dungeon-tiles/preview-graph/{cat}` → `{open_doors, caps_complete}`. pytest `test_issue697_full_doors.py` 6/6. ⚠️ Pełne domknięcie w grze dopiero gdy część A doda 4 zaślepki krypty (dziś `caps_complete:false` → fallback). **Część B2 (endless, #698, 2026-06-16) — ✅ ZROBIONE:** `_fill_open_doors` działał tylko w entry-run; doczepianie segmentu endless (`extend_dungeon_for_endless` / `go_deeper`) miało własną, ślepą logikę styku, która po #697 (boss z 0 wolnymi drzwiami) osierocała zaślepkę bossa i wymuszała drzwi-widmo na kaflu-wejściu (nawrót objawu #697 na styku). Naprawa: `_attach_endless_segment` dobiera kierunek doczepienia z REALNYCH drzwi bossa (preferuje zaślepiony, by nie odpiąć ścieżki), wymaga by `opp_dir` istniał w kaflu-wejściu (brak drzwi-widm), usuwa osierocone zaślepki obu stron i po scaleniu uruchamia `_fill_open_doors` na CAŁYM grafie. Inwariant po `go_deeper`: ZERO null / ZERO drzwi-widm / ZERO sierot / symetria. pytest `test_issue698_endless_door_weld.py` 4/4 + Playwright `issue_698_endless_door_weld`. **Korekta po /playwright-test-report L13c (2026-06-16) — rozbieżność spec↔gra:** pierwsza wersja (commit 78e6ef0) sklejała segment na DOWOLNYCH drzwiach bossa, których przeciwległy kierunek istniał w wejściu — także na drzwiach prowadzących do realnej ścieżki. Na żywym grafie (boss z 2 drzwiami-ścieżki + 2 zaślepkami) urywało to ścieżkę bossa (asymetria) i nadpisywało drzwi-onward wejścia → cały nowy odcinek osierocony (11 sierot, cykl 2). Ostateczna reguła (commit ce9aee8): styk MUSI użyć drzwi **zapasowych** (wolne lub zaślepka) po OBU stronach, naprzeciw siebie; drzwi-ścieżka nigdy nie są użyte. Gdy nie ma zgodnej pary zapasowej → segment jest przegenerowywany (`extend_dungeon_for_endless`, do 12 prób). Zweryfikowane na żywo: cykl 2 (27 węzłów) i cykl 3 (42) — wszystko 0 null / 0 widm / 0 asym / 0 sierot.
...
2. ✅ **Rogue HP == warrior.** ~~`vitality_service` = 10, DB `archetypes` = 8, design = <warrior. Potrójny rozjazd.~~ **Naprawione B2 ([#642](https://github.com/szmidtpiotr/ai-gm/issues/642), 2026-06-15):** `ARCHETYPE_BASE_HP["rogue"]` 10→8 + kreator (`_wizardCalcHP` oraz etykieta karty archetypu) 10→8. DB seed już = 8 (bez migracji). Ordering warrior 10 > rogue 8 > mag 6, wzór HP niezmienny. Backend == kreator. **Drobny dryf poboczny:** DB `game_config_archetypes.hp_base` dla warrior = 12 (nie 10), ale ta kolumna NIE jest czytana przy tworzeniu postaci (char HP liczy `vitality_service`), więc gra niezmieniona — wyrównanie kolumny = osobny task po decyzji Piotra.
3. ✅ **Rogue brak budżetu skilli.** ~~`character_creation_config.py:9/14` — brak klucza `rogue` → fallback na warrior (8/7, bez biasu). Design chce więcej + bias złodzieja.~~ **Naprawione B3 ([#644](https://github.com/szmidtpiotr/ai-gm/issues/644), 2026-06-15):** `SKILL_BUDGET["rogue"]` = {slots:10, active_skills:9} (najwięcej skilli: 9 > warrior 7, ≥ scholar 8) + `ARCHETYPE_SKILL_WEIGHTS["rogue"]` = bias złodzieja (stealth/lockpick/sleight_of_hand/acrobatics/awareness/investigation); `lockpick`+`acrobatics` dodane do `CREATION_SKILL_POOL` (istnieją w `game_config_skills`, bez tego bias byłby martwy). Fallback warrior już nie łapie rogue. Kreator == postać (ten sam endpoint hero-first `POST /characters`). Uwaga: ścieżka kampanijna `POST /campaigns/{id}/characters` nadal wymusza warrior dla rogue (starter loot) — osobny task.
4. ⏳ **Toolkit maga cienki.** ~~10 czarów: atak 6 / heal 1 (self) / tarcza 2 (self) / buff 0 / party 0. Nie może pełnić roli supportu → adopcja `rpg_spells_design_doc.md`.~~ **B6 ([#648](https://github.com/szmidtpiotr/ai-gm/issues/648), 2026-06-15):** zaseedowano pierwszą falę 16 czarów Faza 1 (10→26 w `game_config_spells`) — atak ST ×6, heal-self ×1, self-buff obronny ×2, kondycje ×6 (mapowane na ISTNIEJĄCE stany FAZY S: slowed/cursed/poisoned/blinded/confused/stunned — reużycie, zero duplikatu), narrative ×1. Bez zmian schematu, idempotentny blok w `migrations_admin.py`. Pełny toolkit (tier-gating/DC=B7, start L1=B8, naliczanie kondycji=B9, absorpcja tarcz=B10, AoE=B11) w kolejnych zadaniach Bloku 2.
...
- **Startowy zestaw maga (L1, 5× tier 1):** `fire_bolt` (atak ST) + `minor_heal` (heal) + **`ward_of_iron`** (obrona) + `detect_magic` (utility) + **`spark_burst`** (atak AoE). Pełna tożsamość od startu — w tym opcja obszarowa. ✅ **Wdrożone B8 ([#655](https://github.com/szmidtpiotr/ai-gm/issues/655), 2026-06-15) + B11b ([#983](https://github.com/szmidtpiotr/ai-gm/issues/983), 2026-06-24, dodano `spark_burst`).** **Decyzja D-obronna (Piotr 2026-06-15):** obronny = `ward_of_iron` (tier 1), NIE `mage_armor` (tier 2) — spójność z bramką nauki L1 z B7 (`max_tier=ceil(lvl/2)=1`); dziś oba placeholdery (prawdziwa absorpcja=B10), więc tiebreaker = tier. Migracje `v2-spells-faza-b-b8-starter-backfill` + `v2-spells-b11b-spark-burst-backfill` dosiewają zestaw istniejącym magom NIE-destrukcyjnie (stare czary zostają).
...
| Self-buff AC + pula absorpcji | ✅ absorpcja wdrożona (B10 [#657]) | ward_of_iron 6 / mage_armor 10 temp-HP |
| DoT / over-time | ⚠️ częściowo (kondycje FAZY S) | mapuj na kondycje (Faza 1) |
| AoE multi-target | ✅ **B11 [#659] wdrożone (2026-06-15)** — `_resolve_aoe_spell_in_combat`: 1×d20 vs cel główny, hit→kość dmg per cel; aoe=1=wszyscy żywi, aoe=0=maks 3 (chain). **B11b [#983] (2026-06-24): `spark_burst` (T1, 1d4, aoe=1, mana 3) — najtańszy AoE, w starterze maga** | spark_burst/burning_arc/chain_lightning/fireball działają |
...
| Reakcje (blink, mirror redirect) | ✅ **B16 [#822] wdrożone (2026-06-21)** — MVP = STAN PREWENCYJNY (decyzja akceptacji #1; reuse szkieletu S15/B10, BEZ interrupt-window między turami). `spell_type='reaction'` → `_resolve_reaction_spell_in_combat` zapisuje stan na combatancie gracza; `_try_spell_reaction` odpala go PRZED obrażeniami, **wcześniej niż okno dodge/block** (jedna reakcja/rundę — negujący cios czar pomija dodge/block) | `mirror_image` (N ŁADUNKÓW anulowania, każdy cios -1 → 0 dmg) · `blink` (% szansy pudła d100 przez N rund) · `globe_invulnerability` (N rund nietykalności, 0 dmg, bez zużycia ładunku). Priorytet: globe > mirror_image > blink. Profil w `effect_json.reaction` (charges/miss_chance/rounds). Liczby = startowe (Sandbox-tunable): mirror 3 ład. (T2/3many), blink 50%/3r (T3/4many), globe 2r (T5/6many). **Out of scope:** prawdziwy interrupt poza turą; reakcje WROGÓW (symetria); reakcje sojuszników/summonów. Frontend wiring = follow-up. |

### AK.6 — Decyzje do potwierdzenia (NIE zapomnieć)

- **D1 — HP:** 10/8/6 (rekomendacja, bez retune wrogów) **vs** 12/10/8 (+ re-tune wrogów #475, atak +1). → przyjęto roboczo **10/8/6**. **Wdrożone w silniku+kreatorze (warrior 10 od zawsze, mag 6 od zawsze, rogue 8 = B2 [#642]).** Ostateczne potwierdzenie wartości — po playteście B5/B13.
- **D2 — Rogue sygnatura:** ✅ **ROZSTRZYGNIĘTE (Piotr 2026-06-15): sneak attack jako cecha klasy łotrzyka** (+1d6 z ukrycia), NIE „zostaw generyczny hidden". **Wdrożone B4 ([#645](https://github.com/szmidtpiotr/ai-gm/issues/645)):** helper `_sneak_attack_bonus(sheet)` (rogue-only) w `combat_service` dolicza `sneak_attack` PONAD generyczny `ambush_bonus` z kondycji `hidden` (S19 #614 — reużycie okna zasadzki, zero duplikatu stanu), raz, po mnożniku (nie podwajany na nat20). Wartość startowa kości = `1d6` (płaska; kandydat na skalowanie z poziomem po B5/B13).
- **D3 — Mag CHA-czary:** ✅ **ROZSTRZYGNIĘTE (Piotr 2026-06-21): czary maga wchodzą na INT** (charm_person, mass_fear), spójnie z resztą D-spell — NIE CHA. Casting stat = `INT_mod` w pojedynku przeciwnym (`resolve_spell_opposed_save`); CHA pozostaje czysto społeczna (poza walką). **Wdrożone B17 ([#823](https://github.com/szmidtpiotr/ai-gm/issues/823)):** `charm_person` (`spell_type=effect` → kondycja `charmed`, reuse `_resolve_effect_spell_in_combat`) + `mass_fear` (NOWY `spell_type=effect_aoe` → kondycja `feared` na WSZYSTKICH żywych wrogach, pojedynek INT vs WIS per wróg, mana pełna bez zwrotu, zero obrażeń). Kondycja `feared` = behavior_override `flee` + WIS -3 + rzut WIS DC 14 (start tury). `charmed` zostaje LITE (FAZA S) — B17 ją tylko wyzwala.
- **D-spell — Model trafienia czaru:** ✅ **ROZSTRZYGNIĘTE (Piotr 2026-06-15).** Pierwotny pomysł „stały DC rzucania 10/14/18" (AK.4) **odrzucony** — był niesprawiedliwy (mag tracił manę na każdym pudle, wojownik nic) i błędny (cel bronił się unikiem DEX, więc tępy goblin „unikał" klątwy umysłu zwinnością). **Nowy model:**
>   1. **Czary atakujące** (fire_bolt itd.) — BEZ ZMIAN: `d20 + INT vs unik wroga` jak broń (mana = koszt większej kości; darmowy kantryp 1d4/0 many = zawór, parytet z wojownikiem). **#475 nietknięte.**
>   2. **Czary nie-atakujące** (kontrola/klątwa/oślepienie/spowolnienie) — **pojedynek przeciwny**: `d20 + INT_mod (mag)` vs `d20 + stat_obrony_mod (cel)`. Stat obrony wg efektu (mapowanie na kondycje FAZY S): **WIS** = umysł (confused, cursed, blinded, charmed) · **CON** = ciało (slowed, frozen, stunned, poisoned). Remis → mag wygrywa. Reuse #584 opposed stats + `_combatant_stat_modifier(enemy, stat=…)` (wrogowie mają 7 statów z `actor_stats.py`: brute INT 7/WIS 8 = tępy łatwy cel, caster 14 = oporny).
>   3. **Ekonomia many:** czar łapie → pełna mana; **wróg się oparł → zwrot ½ many** (czar się nie uformował, kara = tylko tura, jak pudło wojownika); miscast (Nat 1) → pełna mana.
>   4. **Tier** → bramka nauki `max_tier=ceil(level/2)` + koszt many; **NIE** stały DC. Waluta nauki/upgrade = **XP** (75/50/100; `arcane_points` martwe — kandydat do usunięcia).
>   Wdrożenie: notes.md FAZA B B6a (bugi flow) → B6b (narracja) → B6c (UI dropdown) → B7 (bramka tieru + pojedynek + zwrot many).
...
**Blok 1 domknięty (2026-06-15):** B1 [#624] staty · B2 [#642] HP · B3 [#644] skille · B4 [#645] sneak attack · B5 [#646] siatka regresyjna 3 klas (`test_issue475_combat_balance.py::test_b5_*` — HP ordering 10>8>6, bonusy statów, rogue 9 skilli > warrior 7, mag w melee ginie ≥100%, spójność kreator↔postać). Liczby D1 (10/8/6) nadal **robocze** — ostateczne potwierdzenie po playteście B13.

**Blok 2 — postęp (2026-06-15):** B6 [#648] seed 16 czarów · **B6a [#649]** flow ataku czarem (zawis tury `combatBusy`/`finally` + animacja kości) · **B6b [#650]** narracja czaru = magia, nie broń (`_user_text_for_llm_context` gałąź spell) · **B6c [#651]** atak czarem jako rozwijane menu przy „Atak" (czary atakujące pod Atak, nie-atakujące pod Akcja→Zaklęcie) · **B7 [#652]** tier-gating nauki (`max_spell_tier_for_level`) + model trafienia D-spell jako helpery (`spell_save_stat` WIS/CON, `resolve_spell_opposed_save`, `mana_refund_on_resist`). **B8 [#655]** startowy zestaw maga L1 = `fire_bolt`+`minor_heal`+`ward_of_iron`+`detect_magic` (4× tier 1; decyzja D-obronna: ward_of_iron, NIE mage_armor) + migracja istniejących scholarów. **B9 [#656]** wpięcie kondycji+pojedynku do walki — gałąź `spell_type=='effect'` w `resolve_attack` → `spell_service.resolve_combat_effect_spell` (pojedynek INT vs WIS/CON z B7, NIE unik); łapie→`apply_condition_to_combatant` (kondycja FAZY S, reuse), opór→½ many zwrot, Nat1→miscast pełna mana; czar kontroli bez obrażeń; 6 czarów (frost_grip/hex/poison_touch/blind/confusion/stun_bolt), `sleep`→`sleeping` wykluczony (brak kondycji w FAZIE S). **Model trafienia D-spell przeszedł z helperów (B7) do żywego silnika.** **B10 [#657]** prawdziwa absorpcja tarcz — gałąź `spell_type=='defense'` w `resolve_attack` → `_resolve_defense_spell_in_combat` nakłada pulę `absorb_hp` na combatancie gracza (NIE stackuje, combat-scoped); `_apply_absorption` soakuje obrażenia wroga PRZED HP (po uniku/bloku); `spell_service.defense_absorb_amount` czyta `effect_json.absorb`. **Dodano brakującą kolumnę `effect_json` do `game_config_spells`** (`v2-spells-effect-json-col`) — wcześniej nie istniała; ward_of_iron=6, mage_armor=10 temp-HP (wartości startowe). Przy okazji naprawiony ukryty bug: rzut czaru obronnego w walce wpadał w ścieżkę ataku (2d6). **B12 [#658]** czary w Kreatorze AI (Smart Entry) + pełny ręczny formularz — **audyt: backend `/api/admin/spells` CRUD i Smart Entry `game_config_spells` już istniały od B6/B7**, B12 domknął FRONTEND: wpięcie `game_config_spells` do `SE_TABLE_LABELS`+mapy tabów (Kreator AI działa z zakładki Czary) + `_openSpellForm` rozszerzony do pełnego schematu (`spell_type`/`effect_stat`/`effect_type`/`effect_duration`/`aoe`/`rank2_json`/`rank3_json`; `target_zone`→any/self/engaged/ranged). Bump `?v=40`. **#595 [review]** wybór celu ataku w walce z wieloma wrogami (czysty helper `_select_player_target` honoruje `target_id`, melee bramkowane strefą, fallback=auto; front: klikalne wiersze + 🎯) — **odblokowuje B11**. **B11 [#659]** AoE multi-target — gałąź `spell_type=='attack_aoe'` w `resolve_attack` → `_resolve_aoe_spell_in_combat`: 1×d20 vs główny cel (#595), hit → osobna kość dmg per żywy wróg; `aoe=1` (obszar: wszyscy) / `aoe=0` (łańcuch: maks 3 via `_AOE_CHAIN_MAX_TARGETS`); Nat1→miscast, Nat20→2× per cel; loot/XP/death-log per wróg (wzorzec single-target). 10/10 pytest GREEN. **Frontend już gotowy** (blok `attack_aoe` w `_handleCombatAttackResult` renderuje `aoe_hits[]`). Zostało: B13 (playtest mag solo → `[SMOKE] FAZA B`).
