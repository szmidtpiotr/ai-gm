...
- #504 — C10 QUEST_SUGGEST integracja: 5 testów integracyjnych pipeline (parse→DB→snapshot) — potwierdza że C10 działa w ścieżce streamingowej i quest ląduje w `game_sessions.active_quests`. Bug z poprzedniej sesji był spowodowany starym buildem kontenera, nie błędem w kodzie. TDD 5/5 + Playwright 2/2. ✅
