...
- Limit: 25 tur/akcji na run (loch jest dłuższy niż scena kampanii). Po limicie: raport, nawet niekompletny.
...
**Bohater:** warrior (domyślnie) — loch jest combat-heavy. Dla testu skrzyń/zagadek rogue (DEX) bywa lepszy.
...
| 5 | Walka na kafelku: start deterministyczny, skala D1–D5 (bez rubber-bandingu) | L4/L5 | poziom wroga = `TIER_ENEMY_LEVELS[tier]`, NIE skalowany do poziomu bohatera; HP przed/po |
...
ruch, mapa, walka, boss, endless, śmierć, porzucenie, flaga) graj normalnie.
...
brak feedbacku) · **P2** = kosmetyka. Priorytet w tytule.
...
**Loch:** key | **Bohater:** id | **Akcji zagranych:** N | **Cykli endless:** M
...
- Walka w lochu deterministyczna z silnika (Decyzja 4) — żadnych tagów COMBAT_START od LLM; jeśli
  LLM próbuje narrować walkę bez startu silnika → defekt.
- Reakcje uniku/bloku (SF10): wróg trafia → modal Przyjmij/Unik/Blok; w smoke wybieraj świadomie i
  sprawdzaj, że działa też w lochu (regresja SF10 w kontekście dungeon combat).
- Nie wymuszaj checkpointów łamiąc fikcję — graj naturalnie; checkpoint nieosiągnięty organicznie
  po limicie → N/D z powodem, to też wynik.
