...
| F-11 | Kreator postaci | `#character-wizard-screen` | 5 kroków: Krok 0 wybór rasy (Człowiek/Krasnolud) → tożsamość → staty → umiejętności → finalizacja (#976 R7) | ✅ front-v2 `CreateCharacter.tsx` (KROK 3 #1232): Rasa→Tożsamość→Cechy(+/- z modami+HP/Mana/Init)→Umiej.→Finał; POST /characters→finalize-sheet; pula skilli z serwera; #1479: karta rasy wyszarzona (🔒 + powód) gdy jej kraina ojczysta nie jest `live` — dostępność z `GET /creation/races`, tester widzi też krainy `coming`; #1477: krok Tożsamość filtruje archetypy per rasa (krasnolud bez Łotrzyka — `blocked_archetypes` z serwera, fallback `RACE_BLOCKED_ARCHETYPES`), zmiana rasy resetuje zablokowany wybór na Wojownika; #1476: karta rasy **Wyspiarz** (⚓, diaspora bez kotwicy — zawsze dostępna), archetypy Kombinator (rogue) / Zabijaka (warrior), Uczony i gish zablokowane |
...
| F-21 | Karta postaci (panel) | nagłówek / swipe | zakładki Staty / Ekwipunek / Czary (mag) / Wygląd; swipe-down zamyka tylko gdy lista na szczycie (scrollTop guard #1091); overscroll-behavior:contain blokuje pull-to-refresh | ✅ front-v2 (KROK 5 #1234): panele = zakładki w grze (nie osobny ekran). 5 paneli — Postać/Umiejętności/Czary/Ekwipunek/Reputacja&opis. Przełącznik: desktop lewy pionowy rail (`components/game/GameRail.tsx`), mobile górny poziomy scroll (`components/sheet/CharacterSheet.tsx`) + dolny tabbar (`shell/TabBar.tsx`) — wszystkie sterują `appStore.gameTab`. Wspólna definicja zakładek `components/sheet/tabs.ts`. **#1510 — katalog czarów filtrowany rasą po stronie backendu:** `useSpellCatalog(enabled, characterId)` woła `/spells?character_id=`, więc zakładka Czary i modal awansu (`AdvancementScreen`) widzą wyłącznie szkołę rasy bohatera (arkana / Rdzeń / Stroiciel); klientowy `canRaceLearnSpell` zostaje jako druga linia obrony. **#1522 — bramka umiejętności klasa+rasa:** `usePublicSkills(enabled, characterId)` woła `/mechanics/skills?character_id=`, a kreator (`StepSkills`) filtruje kandydatów do zamiany przez `skillAllowed(key, archetype, race)` z `lib/creation.ts` (lustro `skill_access_service`). **#1523 — redesign kroku UMIEJĘTNOŚCI:** natywny `<select>` zastąpiony `components/game/creation/SkillPickerSheet.tsx` (bottom-sheet/side-panel z wyszukiwarką bez-diakrytyczną, grupowaniem po cesze i opisem z katalogu); wiersz umiejętności rozwija opis (`aria-expanded`), rangi po polsku (`RANK_LABEL`), lista kandydatów wyłącznie z żywego katalogu — statyczne `ALL_SKILL_ROWS` służą już tylko za etykiety |
...
| F-27 | Awans (XP direct-spend V2) | przycisk ⬆️ Awansuj | wydawanie PD | ✅ front-v2: `components/game/outcomes/AdvancementScreen.tsx` wg `TASK_25_XP_PROGRESSION_V2` [D12] — **BRAK auto-modala** (poziom = tylko etykieta). Awans RĘCZNY: badge ⬆️ „Awans" w pasku gry (widoczny gdy `xp_available ≥ 30`) + wpis w menu ☰; store `advancementOpen`. Overlay (bottom-sheet mobile / prawy panel desktop) z 3 zakładkami: **Statystyki** (7 cech, koszt `stat_point_costs[newVal]`, `spend-stat`), **Umiejętności** (katalog `/mechanics/skills` × rangi z sheet, koszt `rank_up_costs[nextRank]`, `spend-skill`), **Magia** (Scholar: `spend-spell-learn` 75 PD / `spend-spell-upgrade` R2=50/R3=100). „Dostępne PD" u góry, blokada podczas walki. Zastąpił auto-modal `LevelUpGate/LevelUpModal` (usunięte). |
| F-28 | Ekran śmierci | HP ≤ 0 | czaszka, epitafium, akcje (nowa przygoda/świat/bohater/wskrzeszenie) | ✅ front-v2 (FE10 #1237): `components/game/outcomes/DeathScreen.tsx` — wywoływany `combat.ended_reason='player_dead'`. Czaszka + epitafium (`GET /campaigns/{id}/death-summary`) + statystyki (poziom/tury/dni z zegara) + akcje: Wskrześ (`GET resurrect-preview`→`POST /characters/{id}/resurrect`, gate na enabled+cost) / Nowa przygoda tym bohaterem (→kampanie) / Inny bohater (→bohaterowie). Makieta zar7-smierc |
| F-29 | Ekran zwycięstwa | boss pokonany (loch) | laur, tytuł zakończenia, statystyki, akcje | ✅ front-v2 (FE10 #1237): `components/game/outcomes/DungeonVictoryModal.tsx` — wywoływany `dungeon_run.boss_choice_pending` (`GET /campaigns/{id}/dungeon-run` po victory). Korona + tytuł + statystyki (XP/złoto/pokonani/pokoje) + Zejdź głębiej / Wyjdź z łupem → `POST /dungeons/boss-choice`. Makieta zar7-zwyciestwo |
| F-30 | Koniec walki | walka wygrana (poza lochem) | ikona zwycięstwa, podsumowanie łupu, "Kontynuuj" | ✅ front-v2 (FE10 #1237): `components/game/outcomes/CombatEndModal.tsx` (scalony z F-31) — wywoływany `combat.ended_reason='victory'`. Puchar + tytuł/podtytuł (pokonani · N rund) + staty (+XP/+złoto/HP z akumulatorów `resolve-attack`) + pasek XP z przyrostem (`xpProgress` na progach domyślnych). Orkiestracja: `outcomes/CombatOutcomes.tsx` (montowany w `CombatView.tsx`). Makieta zar7-koniec |
| F-31 | Popup łupu | zwycięstwo w walce | siatka itemów, "Weź łup" / "Pomiń" | ✅ front-v2 (FE10 #1237): część `CombatEndModal.tsx` — siatka łupu z `combat.loot_pool` (ikony wg typu item/consumable/weapon, ramka rzadka), Weź wszystko → `POST /combat/loot/claim` (wszystkie indeksy) / Pomiń. Makieta zar7-koniec |
| F-32 | Drop celebration | rzadki/affixed drop (U17) | karta rzadkiego itemu, diff vs założony | ✅ front-v2 (FE10 #1237): `components/game/outcomes/DropCelebration.tsx` — wywoływany gdy `claim` zwróci `comparison.is_special`. Orb z obrotowymi promieniami + nazwa/rzadkość + afiksy jako opis + diff vs założony (`new`/`equipped`/`diff` z `build_drop_comparison`, ▲/▼/=) + Załóż teraz (`POST /inventory/{id}/equip` na `suggested_slot`) / do plecaka. Makieta zar9-drop |
...
| F-51 | Log narracji | dymki gracza/GM/system, metadane (nazwa/tura/data — toggle), fade-in, auto-scroll, slash-popup; karty combat_turn (⚔️ atak gracza / 🗡️ atak wroga / 💀 śmierć / 🛡 reakcja). **#861 (dual-wield):** off-hand drugi cios → karta „🗡️🗡️ DRUGI CIOS" (klasa `cturn--offhand`); atak wroga sparowany → badge „🛡 Parujesz (+N obrona)" (`cturn--parried`) — flagi `offhand`/`parry_bonus` z meta combat_turn (silnik #598) | ✅ front-v2 `components/game/NarrationLog.tsx` (KROK 4 #1233): dymek GM (serif, tło, „MISTRZ GRY · tura N") · akcja gracza prawo/ember/italic · systemowe=kreskowany pasek · „GM pisze…" (submit pending) · fade-in + auto-scroll; proza escapowana z *kursywą*/«cytatami»→em (bez XSS); parser `buildLog`. **Dialogi kolorem** (`renderNarration`, per-akapit): kwestia NPC `— …` → `.speech` (złoto+kursywa+lewy obrys) = mowa; cytat pisany „…" → `.quote` (gold-glow kursywa) = tekst czytany; wg `system_prompt.txt §FORMAT` |
...
| F-83 | Kolekcje: Bestiariusz + Atlas Kresów (ŻAR) | zakładka **Kolekcje** w karcie postaci (rail + tabbar, `SHEET_TABS`) | Nowa zakładka karty (`PanelCollections.tsx`) z przełącznikiem **Bestiariusz \| Atlas Kresów**. Bestiariusz: siatka kart pokonanych typów wrogów (portret `image_url`, licznik zabójstw, pasek progresu do progu, badge tieru 📖/👁 HP/⚔ +1); nieodblokowane = sylwetka „???" (zero wycieku nazwy z API); klik → modal z opisem/lore + HP (tier ≥2). Atlas: kafle eksploracji (odkryte heksy + % świata, lokacje, plotki potw./niesprawdzone, liczba krain), rozbicie per kraina, lista plotek (✓ potwierdzone). Cross-kampanijne per bohater. Fetch: `useBestiary`/`useAtlas` (`/characters/{id}/bestiary`, `/atlas`), invalidacja `["bestiary"]` po każdej akcji walki. Wiedza łowcy 1/5/15 zabójstw → wpis/HP/+1 do trafienia | #1191 |
| F-84 | Zegar świata: widget + popup „Łuk dnia" (ŻAR) | klik w zegar w pasku przygody (`Topbar` GameBar) lub w nagłówku zakładki Mapa (`WorldMap`) | Widget zegara ożywiony (A): crossfade słońce↔księżyc + kolor akcentu wg pory dnia (`phaseTheme`), mini-ikona pogody obok pory (Cloud/CloudRain/…), fade przy zmianie godziny; ambient (C): delikatny inset-poblask krawędzi paska wg pory. Klik → popup **`DayArcModal.tsx`** (B): półokrąg horyzontu z gradientem nieba per faza, słońce/księżyc na łuku wg godziny (`arcProgress`/`arcPoint`, animowane wejście), odczyt `HH:MM · faza · Dzień N · 🍂 pora roku`, karta pogody (ikona + etykieta PL + chip „marsz wolniejszy" gdy `slows_travel`), podpowiedź nocnej ekonomii (`night_hint` ≤3h do nocy), atmosfera CSS (deszcz/śnieg/mgła). Helpery `lib/worldClock.ts`. Backend: `GET /campaigns/{id}/clock` wzbogacony o `season`/`weather`/`is_night`/`hours_to_night`/`night_hint` (dane z FAZY PT, zero nowych tabel) | #1219 |
| F-85 | Rzemiosło u rzemieślnika (ŻAR) | deterministyczny chip **„Rzemiosło" 🔨** w akcjach (`OPEN_CRAFTING:<locationKey>`, jak „Usługi") — omija LLM | Modal **`CraftingOverlay.tsx`** (lokacyjny, nie npc-owy jak sklep): nagłówek rzemieślnika (kowadło 🔨 + nazwa lokacji + złoto gracza), lista przepisów jako karty. Każda karta: ikona wg typu (miecz/klucz/liść/fiolka), badge typu wyniku (Mikstura/Ulepszenie broni/Naprawa pancerza), **podświetlenie zielone + badge „Starczy"** gdy bohater ma wszystkie komponenty (`hasAllComponents` po ekwipunku), chipy składników 🧩 z licznikiem **have/need** (zielony gdy starczy, czerwony gdy brak), opłata usługi, przycisk **Wytwórz** (blokada „Brak komponentów"/„Za mało złota"). Po craftcie: toast (+ „zniżka krasnoluda" gdy dotyczy) i invalidacja `["character"]`+`["inventory"]` → liczniki komponentów i złoto odświeżają się na żywo. Fetch: `useLocationCrafting`/`useCraft` (`GET /locations/{ref}/crafting`, `POST /characters/{id}/craft`). Store: `crafting` + `openCrafting`/`closeCrafting`. Backend: chip w `suggested_actions.py` (`location_has_crafting`), rdzeń craftu z #1336 BL-C1 | #1338 |
| F-86 | Eksperymenty rzemieślnicze: ukryte receptury + fuszerki (ŻAR) | zakładka **Eksperyment** 🧪 w `CraftingOverlay.tsx` (obok „Przepisy") | Panel eksperymentu w modalu Rzemiosła: baner ostrzegawczy o ryzyku, lista stosów z ekwipunku z **checkboxem wyboru + stepperem ilości** (limit **2–4** składników), pasek akcji z licznikiem `N/4 wybrane`, opłatą 🪙 10 zł i przyciskiem **Eksperymentuj** (blokada „Wybierz 2–4"/„Za mało złota"). Po próbie **dramatyczna karta wyniku** (`data-outcome`): **Odkrycie!** (złota, przepis + item), **Receptura się wymyka** (porażka, strata połowy), **Fuszerka** (czaszka, strata/1k4/przeklęty) — z rzutem 🎲 raw+mod=total vs DC, znacznikami Nat 20/Nat 1 i ubytkiem HP. Sekcja **Odkryte receptury** (pigułki złote) z `useCharacterRecipes`. ZASADA ŻELAZNA: wynik zawsze z receptury admina (`is_hidden=1`) — kombinacja spoza puli daje tylko fuszerkę. Fetch: `useExperiment`/`useCharacterRecipes` (`POST /characters/{id}/craft/experiment`, `GET /characters/{id}/recipes`). Backend: `crafting_service.experiment` — sygnatura kombinacji → match ukrytych receptur → test `trade_craft` DC wg tieru | #1341 |
| F-87 | Karta pojawienia wroga: obrazek + nazwa + relatywny wskaźnik zagrożenia | wejście w walkę (pierwszy render danego `combat_id`) | Overlay `EnemyRevealCard.tsx` (wzorzec `TileImageModal` z lochu): przy **wejściu w walkę** odsłania portret herszta (`image_url`, fallback ⚔ „Brak wizerunku"), nazwę, `i N innych` przy grupie oraz **relatywny wskaźnik zagrożenia** 🟢/🟡/🔴/💀 + label (pill lewy-górny). Once-per-`combat_id` (`revealedRef`, jak `seenTiles` w `DungeonView`). Dismiss (✕ / tło / **Stań do walki**) → normalny widok. Mały glyph+label wskaźnika też w `CombatBanner` przez całą walkę (refresher do decyzji o ucieczce). **Surowa liczba NIGDY nie pokazana** — tylko glyph+label (parytet z ukrytym Power Score). Backend: `threat_display_service.relative_threat` = Σ `enemy_threat_value(stały statblock)` × liczebność / `threat_budget_for_power(power)`; progi w `game_config_meta.threat_indicator_bands` (STARTING); `image_url` + `relative_threat` wpięte do snapshotu walki (`_row_to_combat_dict`, JOIN po `enemy_key`). Staty wroga STAŁE — wskaźnik nie rubber-banduje, wizualizuje progres Power Score | #1344 |
| F-88 | Karta inicjatywy: oba rzuty + kto zaczyna | start walki (pierwszy render danego `combat_id`) | Overlay `InitiativeCard.tsx` (wzorzec `EnemyRevealCard`): na starcie walki pokazuje OBA wyniki rzutu inicjatywy — **Ty vs Wróg** (dwie karty `d20`, wyższy rzut = amber glow) + werdykt **„Zaczynasz!"/„Zaczyna wróg"** (`data-testid="initiative-starter"`). Grupa wrogów → wiodący (najwyższa inicjatywa) + `+N`. Once-per-`combat_id` (`useInitiativeCard` latchuje starter z `turn_order[0]` PRZY pierwszym aktywnym snapshotcie — zanim auto-driver tury wroga przesunie `current_turn`). Renderuje się PO karcie pojawienia wroga (`!reveal`) → dwa overlaye się nie nakładają; zasadzka (suppressReveal) → od razu. Dismiss (✕ / tło / **Do walki**). Dane już w snapshotcie (`initiative_roll` na kombatantach + `turn_order`, `combat_service.py`) — fix był czysto prezentacyjny. Logika domknięta unit-testem `useInitiativeCard.test.tsx` (renderHook) | #1356 |
| F-89 | Zakładka Receptury: znane receptury pogrupowane po secie + liczniki komponentów | pojawia się po nauczeniu **pierwszej** receptury (loot/eksperyment); pusta wiedza = brak zakładki | Panel `PanelRecipes.tsx` (zakładka `recipes` w `SHEET_TABS`, gating `visibleSheetTabs(hasMana, hasRecipes)` — `has_any` z `GET /characters/{id}/recipes`). Grupuje karty po **secie** (`set_label` + pasek postępu + „jeszcze X nieodkrytych" jako sylwetki `Lock` bez nazw); receptury bez setu → „Luźne receptury". Każda karta: wynik + tier + **liczniki posiadanych komponentów** (`owned/qty`, zielone gdy starczy) + 2 przyciski: **Wykonaj sam** (`mode=self`, wymaga `trade_craft`≥1 dla loot) i **Zleć rzemieślnikowi** (`mode=service`, koszt ×1.5, bez skilla). Receptura z lootu = auto-nauka (nie do plecaka); duplikat → sprzedawalny „Zbędny zwój receptury". Gating wpięty do railu (`GameRail`), tabbara (`TabBar`) i karty (`CharacterSheet`). Unit-test `tabs.test.ts` (widoczność zakładki) | #1375 |
| F-90 | Szybkie akcje: chipy LLM pod composerem + toggle per-user | chipy nad polem tekstowym; toggle w Profilu | Chipy renderuje `Composer.tsx` (rząd pill nad textareą), dane z `suggested_actions` (POST /turns) + `GET /campaigns/{id}/suggested-actions`; `normalizeChips()` niesie `source` (`lib/game.ts`). Backend `suggested_actions.py`: chipy **regułowe** (podróż/odpoczynek/usługi) zawsze, chipy **LLM** domieszane tylko gdy globalna flaga `quick_chips_enabled` (game_config_meta), z twardym filtrem zagadek/testów (`fold()` #1420) i capem `quick_chips_max` (start 3); LLM-owe oznaczone `source:"llm"`. **Per-user toggle**: `Profil → Rozgrywka → Szybkie akcje` (`ToggleRow` w `Profile.tsx`, hooki `useQuickChipsPref`/`useSetQuickChipsPref` → `/users/{id}/preferences/quick-chips`); gdy off, `Game.tsx shownChips` filtruje `source==="llm"` (regułowe zostają). Admin: `System → LLM → ⚡ Szybkie akcje`. Prompt: sekcja „SUGEROWANE AKCJE" w `system_prompt.txt` (max 3, zakaz zdradzania zagadek) | #1215 |
| F-91 | Towarzysze podróży + wierzchowce (karta) | sekcja „Towarzysze" w zakładce Postać | Sekcja `CompanionsSection` w `PanelCharacter.tsx` (widoczna gdy `useCompanions` zwraca ≥1). Każdy towarzysz: ikona wg typu (🐴 mount / 🐕 animal / 🗡 hireling), imię, badge własny/najęty, pasek HP (tylko walczący), ostrzeżenie underfed/zaległy żołd, przycisk zwolnij (`useDismissCompanion`). Dane: `hooks/useCompanions.ts` (`useCompanions`/`useLocationCompanions`/`useHire`/`useBuy`/`useDismiss`), typy `lib/companions.ts`, endpointy `/characters/{id}/companions/*` + `/locations/{key}/companions`. Backend #1192 (companion_service). **TODO w TW9**: modal rekrutacji w stajni/karczmie, chip inicjatywy towarzysza w walce, przycisk „Uciekaj konno" (TW7) | #1192 |
| F-92 | Rejsy: modal „Wypłyń" w porcie (fast-travel port↔port) | chip **„Wypłyń" ⚓** w porcie (Czarnogród/Zatoka) → overlay | Modal `SailOverlay.tsx` (wzorzec `ServicesOverlay`, slot `sail` w `appStore`, otwierany przez `onChip` prefiks `OPEN_SAIL:<portKey>`). Krok „browse": lista tras z bieżącego portu (cel, **opłata 🪙 / czas ⏱ / ryzyko**), baner nocy (ryzyko rośnie) + baner „Masz Mapę Smolnego" (mniej raf, skróty), znacznik **skrót** przy trasach ukrytych bez mapy; przycisk **Wypłyń** blokowany „Za mało złota". Krok „done": karta dotarcia (cel, czas, zapłacono/zostało) + karta zdarzenia rejsu (⚡ sztorm / 💀 rafa / ⚠ piraci) z narracją. Fetch: `hooks/useSeaVoyage.ts` (`useSeaRoutes` GET `/campaigns/{id}/sea-routes`, `useSetSail` POST `/campaigns/{id}/sail`). Backend: `sea_voyage_service` + `api/sea_voyage.py` — pobiera złoto (`economy_service.change_gold`), przesuwa zegar (`clock_service.advance_clock`), jeden rzut na zdarzenie (noc ×1.5, Mapa Smolnego tłumi rafę), skok pozycji do portu docelowego. Zatoka to wyspa → bez rejsu nieosiągalna | #1504 |
| F-93 | Wskaźnik pływu na wybrzeżu (Wybrzeże Łez) | gdy bohater stoi na hexie wybrzeża (`plycizna`/`rafy`/`morze`/`wydmy`/`coast`) | Pasek `TideIndicator.tsx` (nad mapą w `Game.tsx`, sam się chowa poza wybrzeżem — `on_coast`). Pokazuje **fazę** (Odpływ = morski/cyan, Przypływ = amber/groźny) + tekst „płycizna przejezdna" / „zalana, nie wejdziesz"; gdy stoisz na mieliźnie przy przypływie → ostrzeżenie **„Uciekaj z mielizny!"**. **Licznik godzin do zmiany** widoczny TYLKO z **Tabliczką pływów** (`has_board` → `hours_to_change`); bez niej kursywa „nie wiesz, ile do zmiany" (kanon §6). Fetch: `hooks/useTides.ts` (`useTide` GET `/campaigns/{id}/tide`). Backend: `tide_service` (faza z zegara: 6 h/fazę = 2 cykle/dobę) + `api/tides.py`. Blokada wejścia na `plycizna` przy przypływie i łagodne wyrzucenie zastanego (przeniesienie + drobne HP) siedzą w `hex_travel_service.execute_travel` / `rest_service` | #1504 |
