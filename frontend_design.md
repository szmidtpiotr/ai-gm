...
| F-11 | Kreator postaci | `#character-wizard-screen` | 5 kroków: Krok 0 wybór rasy (Człowiek/Krasnolud) → tożsamość → staty → umiejętności → finalizacja (#976 R7) | ✅ front-v2 `CreateCharacter.tsx` (KROK 3 #1232): Rasa→Tożsamość→Cechy(+/- z modami+HP/Mana/Init)→Umiej.→Finał; POST /characters→finalize-sheet; pula skilli z serwera; #1479: karta rasy wyszarzona (🔒 + powód) gdy jej kraina ojczysta nie jest `live` — dostępność z `GET /creation/races`, tester widzi też krainy `coming` |
