...
| F-27 | Level-up | próg XP | alokacja statystyk | ✅ front-v2 (FE10 #1237): `components/game/outcomes/LevelUpModal.tsx` + wykrywanie `LevelUpGate.tsx` (nasłuch wzrostu `sheet.level` — DOWOLNE źródło XP: walka/quest/odpoczynek, montowany w `Game.tsx`). Medal + „Poziom N" + przyrosty (HP=CON_mod / mana=INT_mod maga / +1 pkt tajemny) + rozdział punktu cechy (6 cech bez LCK, klik→spend-stat PD gdy stać) + odblokowane czary (tier-gate `tierUnlockLevel`, nieznane). Makieta zar9-levelup |
