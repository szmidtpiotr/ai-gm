...
**918-A2 per-plik (LIST `fix_a2_files.md`):** każda linia = ścieżka pliku testowego (`tests/...`) do migracji fixtur. Zadanie = zazielenić jego testy naprawiając WYŁĄCZNIE dryf fixtur (schemat testowej DB), bez zmiany asercji/logiki/silnika.
- **Recipe:** (1) `import _fixtures_schema as fx` + `sys.path.insert(0, str(Path(__file__).resolve().parent))` (helper `backend/tests/_fixtures_schema.py`, 15 tabel). (2) Dla `game_config_*`: `fx.create_tables(conn, ...)` przed INSERT-ami; usuń inline `CREATE TABLE game_config_*` (INSERT-y zostają). (3) Brakujące kolumny core-tabel wg REALNEGO schematu (`PRAGMA table_info` na `/data/ai_gm.db`): najczęściej `campaigns.mode`, `npcs.is_crafter`, `characters.*`. (4) Brakująca TABELA (`game_items`, `campaign_templates`, `character_spells`, `character_xp_grants`…) — utwórz wg realnego schematu + seed mirrorujący to, co plik już seeduje (żeby asercje stock/cen miały dane). (5) Guard #928 zielony po zmianie.
- **GATE (nie zmieniaj, opisz):** błąd NIE-fixtura: removed-function (`ImportError: cannot import name '_infer_table'`), logika, **asercja damage** (#826 margin / crit+surprise ×4 = klasa #943, decyzja Piotra), feature gap (#323 `execute_ally_auto_attack`/`ally_npc_keys` — nigdy nie istniały). → `MASS_STATUS: GATE — nie-fixtura: <co>`.
- **Pipeline:** pytest tego pliku zielony (lub GATE). NIE /tdd, NIE playwright. Commit per plik, ref #942.
- **Mapowanie:** linie bez #NNN — `FIX<n>` = n-ta linia (plik z linii).
