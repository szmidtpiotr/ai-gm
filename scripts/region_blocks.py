...
(to samo `row`) przesunięcie ekranowe `screen_y_shift = r_off + q_off/2 = 75*row`
...
def block_offsets(col: int, row: int) -> tuple[int, int]:
    ...
    q_off = 50*col ; r_off = 75*row - 25*col  (kompensacja shear flat-top).
    """
    return (50 * col, 75 * row - 25 * col)
...
def screen_y_shift(q_off: int, r_off: int) -> float:
    ...
    Dla wzoru bloków redukuje się do 75*row (niezależne od kolumny) — to
