...
**Konwencja nazw pilnowana po stronie kodu — we WSZYSTKICH generatorach.** Nazwy własne wymyśla w tym projekcie dziesięć miejsc: podpowiadacz gospodarza, Kreator AI, generator sub-lokacji osady, Kuźnia (szablony kampanii, podlokacje, spotkania, przedmioty), plan GM, nowy akt, katalog spotkań, plotki i kafle lochów. Każde miało własną instrukcję i każde mogło popełnić ten sam błąd. Dziś każde dostaje ten sam blok konwencji z `world_naming_service`, w dwóch wariantach: **osoby** (imiona) i **miejsca** (toponimy — rdzeń reguły #997). Pilnuje tego test-guard: generator z listy bez konwencji = czerwony test, a wyjątek musi mieć wpisany powód (narrator ma regułę wprost w `system_prompt.txt`).

**Konwencja per kraina.** Podpowiadacz gospodarza dostaje regułę nazewniczą **tej** krainy (kanon #997 + rozdziały krain: elfy miękko i śpiewnie, krasnoludy nordycko z polskim przydomkiem, Piętnowani z brzmieniem arabskim, dwór Korony archaiczno-dworsko, wyspiarze krótko i samogłoskowo) plus imiona postaci, które **już stoją** w tej krainie. Dwa strażniki łapią typowe wpadki modelu: współczesne polskie imię („Agnieszka Kruk") i skopiowanie podanego przykładu albo NPC-ikony z kanonu (np. „Ravu" z Wybrzeża) — w obu wypadkach prosimy model o drugie podejście, zanim propozycja trafi na ekran. Reguły żyją w `backend/app/services/world_naming_service.py`; zmiana kanonu nazw = świadoma zmiana tego pliku.
