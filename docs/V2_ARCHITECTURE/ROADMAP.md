...
- [-] **F1 — Player account management screen.** "Karta Przygodnika" profile view exists with most of the basics. Friend system foundation is in DB only; account deletion isn't built. Three slices:
  ...
  - [x] **F1.2 — Account deletion with grace period.** Shipped — commit `e0c09f2`, issue [#114](https://github.com/szmidtpiotr/ai-gm/issues/114). `users.deleted_at` column + `POST /auth/delete-account` (JWT + password confirm) + `POST /auth/undelete` (creds only) + login returns 423 with `pending_deletion` + undo deadline for flagged accounts. Frontend: red "Usuń konto" expandable in Bezpieczeństwo, `#undelete-modal` shown on 423-login. **Hard-delete cron deferred** — soft-deleted rows persist until a manual purge or future cron job hits them. Grace period = 7 days (`_DELETE_GRACE_DAYS` in `auth.py`).
