...
- [-] **F1 — Player account management screen.** "Karta Przygodnika" profile view exists with most of the basics. Friend system foundation is in DB only; account deletion isn't built. Three slices:
  - [x] **F1.1 — Self-service basics.** Fully shipped as "Karta Przygodnika" — `frontend/front/index.html:285-442` + `app.js:7529-7782`. Avatar upload (DataURL, 2 MB cap), inline display-name edit, inline @username edit with uniqueness check, email display, change-password card (current/new/confirm + 8-char min), logout button, stats grid (heroes/campaigns/XP/turns), weekly invite quota. Backend: `PATCH /me/profile`, `GET /me/stats` (both `admin.py:3874/3945`), `POST /auth/change-password` (`auth.py:670`). Reached via the settings cog → `loadProfilePage()`.
  - [ ] **F1.2 — Account deletion with grace period.** Soft-delete: button + confirm dialog → `users.deleted_at = now()`, force-logout the session, show a 7-day "Cofnij usunięcie" banner on next login attempt within the window. Daily cron hard-deletes rows where `deleted_at < now() - 7d`. Backend: new `users.deleted_at` column, auth middleware rejects login for soft-deleted accounts (returns a special 410 "soft_deleted" code with the undo deadline so frontend can show the banner). Frontend: red "Usuń konto" button in the Bezpieczeństwo card. ~half day.
  - [ ] **F1.3 — Friend system.** Table foundation exists (`user_friendships(user_a_id, user_b_id, status DEFAULT 'pending', created_at)`, `migrations_admin.py:2825`). What's NOT built: any endpoint, any frontend UI. Endpoints needed: `/me/friends` (list with status filter), `/me/friends/search?q=` (by username/display_name, prefix-match, capped at 20), `/me/friends/request` (POST), `/me/friends/{id}/respond` (POST accept|decline), `/me/friends/{id}` (DELETE = unfriend/block). Frontend: friends list panel in the Karta Przygodnika screen with status chips (pending = ⏳, blocked = 🚫). Prerequisite for **F2 multiplayer chat**. ~1-2 days.

  Out of scope across all three: email verification, 2FA (push to Stage 10 Auth security), social-graph features (followers, public profiles).
