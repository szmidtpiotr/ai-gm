...
- [x] **F1 — Player account management screen.** All three slices shipped. Karta Przygodnika now covers profile basics, soft-delete with grace, and friend system. F2 multiplayer chat unblocked.
  - [x] **F1.1 — Self-service basics.** Fully shipped as "Karta Przygodnika" — `frontend/front/index.html:285-442` + `app.js:7529-7782`. Avatar upload (DataURL, 2 MB cap), inline display-name edit, inline @username edit with uniqueness check, email display, change-password card (current/new/confirm + 8-char min), logout button, stats grid (heroes/campaigns/XP/turns), weekly invite quota. Backend: `PATCH /me/profile`, `GET /me/stats` (both `admin.py:3874/3945`), `POST /auth/change-password` (`auth.py:670`).
  - [x] **F1.2 — Account deletion with grace period.** Shipped — commit `e0c09f2`, issue [#114](https://github.com/szmidtpiotr/ai-gm/issues/114). `users.deleted_at` column + `POST /auth/delete-account` (JWT + password confirm) + `POST /auth/undelete` (creds only) + login returns 423 with `pending_deletion` + undo deadline. Hard-delete cron deferred. Grace = 7 days (`_DELETE_GRACE_DAYS`).
  - [x] **F1.3 — Friend system.** Shipped — commit `5b7a6a9`, issue [#115](https://github.com/szmidtpiotr/ai-gm/issues/115). New `app.api.friends` router: `GET /me/friends` (accepted/incoming/outgoing), `GET /me/friends/search?q=` (prefix-match, cap 20, excludes self/soft-deleted/inactive), `POST /me/friends/request` (auto-accepts reverse-pending), `POST /me/friends/{id}/accept`, `DELETE /me/friends/{id}` (handles decline/cancel/unfriend). `user_friendships` gained `requester_id` + `responded_at`. Frontend Znajomi card with debounced search (250 ms, min 2 chars) + three sections (📬 incoming / ⏳ outgoing / ⚔ accepted). Block flow deferred.

  Out of scope across all three: email verification, 2FA (push to Stage 10 Auth security), social-graph features (followers, public profiles), block/unblock UI.
