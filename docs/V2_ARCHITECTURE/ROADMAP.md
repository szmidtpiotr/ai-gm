...
- [ ] **F1 — Player account management screen.** Standalone "Twoje konto" view accessible from the main menu / settings. Three slices, picked one at a time:
  - [ ] **F1.1 — Self-service basics.** "Twoje konto" screen reachable from the main menu / a settings cog. Cards for: change password (verify old + enter new + confirm; reuses the existing `change_password` endpoint in `auth.py:671`, just needs a player-facing wrapper at `/api/me/password`), change display name (`users.display_name` already exists — `/api/me/display-name`), change email (optional — `/api/me/email`). Gold-accent profile card at the top with avatar placeholder + role badge. Frontend: new `account-screen` div + JS section in `frontend/front/`. ~half day.
  - [ ] **F1.2 — Account deletion with grace period.** Soft-delete: button confirm-dialog → `users.deleted_at = now()` + a 7-day "Cofnij usunięcie" banner shown on next login. Cron-style purge (existing daily backup cron on `.61` can host it) that hard-deletes rows where `deleted_at < now() - 7d`. Logged out everywhere except the device that initiated. Backend: `users.deleted_at` column + auth middleware rejects 401 for soft-deleted accounts unless within grace period. ~half day.
  - [ ] **F1.3 — Friend system.** `user_friendships(user_a_id, user_b_id, status, requested_at, responded_at)` table with `status ∈ {pending, accepted, blocked}`. Endpoints: `/api/me/friends` (list), `/api/me/friends/search?q=` (by username/display_name, prefix-match, capped at 20), `/api/me/friends/request` (POST), `/api/me/friends/{id}/respond` (POST accept|decline), `/api/me/friends/{id}` (DELETE = unfriend/block). Frontend: friends list panel in the account screen with status chips (zielona kropka = online, pending request = ⏳, blocked = 🚫). Prerequisite for **F2 multiplayer chat**. ~1-2 days.

  Out of scope across all three: email verification, 2FA (push to Stage 10 Auth security), social-graph features (followers, public profiles), avatar uploads.
