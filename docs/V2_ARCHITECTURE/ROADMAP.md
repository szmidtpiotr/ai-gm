...
- [ ] **AP4** TASK_33SA: form fields highlight changed-in-last-response (foundation in AP3: `changed_fields` + `_flashChangedFields`)
...
- [ ] **F3 — Accelerometer shake-to-roll dice on mobile ([#66]).** Use `DeviceMotionEvent` to detect a shake gesture and trigger `_diceBox.start_throw()` instead of the current auto-roll. Throw vector derived from shake direction. iOS needs `DeviceMotionEvent.requestPermission()` on first gesture (one-time, stored in localStorage). ~40 lines Android-only, ~80 lines cross-platform. Live-gravity-during-physics is out of scope (needs dice.js fork). See issue #66 for full design notes.
