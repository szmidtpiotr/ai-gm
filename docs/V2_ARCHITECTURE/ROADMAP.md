...
- [x] **F3 — Accelerometer shake-to-roll dice on mobile ([#66]).** Full implementation including secondary scope — commits `5a17448` (base), `283ac57` (directional + haptic), `a44b819` (tilt-aim + manual fallback), issue [#113](https://github.com/szmidtpiotr/ai-gm/issues/113). `_initShakeToRoll` (18 m/s² threshold) + `_initTiltAim` (DeviceOrientation ±30° → vector) + `start_throw_with_vector` in `dice.js`; `navigator.vibrate(60)` haptic on detection; gold crosshair reticle moves with tilt-aim; "🎲 Rzuć ręcznie" pill as always-available fallback; iOS motion+orientation permissions bundled in one tap (`localStorage.aigm_motion_permission` / `aigm_orientation_permission`).
