...
- [x] **F3 — Accelerometer shake-to-roll dice on mobile ([#66]).** Implemented as the shake-to-trigger primary scope — commit `5a17448`, issue [#113](https://github.com/szmidtpiotr/ai-gm/issues/113). `_initShakeToRoll()` attaches `devicemotion` listener with 18 m/s² threshold; iOS permission cached in `localStorage.aigm_motion_permission`; hint card with wiggling 📱 emoji shown while waiting. Desktop & unsupported devices fall through to auto-throw. Tilt-for-throw-direction (secondary scope) deferred.
