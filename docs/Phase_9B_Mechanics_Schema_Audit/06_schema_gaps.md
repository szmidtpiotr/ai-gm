...
| Kampania / LLM (**[S11]**…) | Plan, rollup, scena | `campaigns.gm_plan_json`; `campaign_ai_summaries` (+ `audience`); **POST** `…/gm-plan/advance-scene` w [`campaigns.py`](../../backend/app/api/campaigns.py) | SoT tur: `campaign_turns`; cooldown rollupu w meta; `user_llm_settings.mode` + centralny resolver default/custom po **T27** | **Zgodne po T27:** W1 plan MG + rollup + advance scene są w runtime, T20 dodał heurystykę dywergencji gracza i adminowy UI/endpointy do edycji `gm_plan_json`, a T27 scentralizował wybór providera/modelu/klucza (env/runtime vs profil użytkownika) dla narracji, admina i testów |
