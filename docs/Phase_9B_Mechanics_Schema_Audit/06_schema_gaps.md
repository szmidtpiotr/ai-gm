...
| Przedmioty | Efekty JSON vs płaskie pola | `game_config_items`: **`effect_json`** + jednocześnie kolumny `effect_type` / `effect_*` (migracje 8H) | Docelowo jeden schemat **[S13]** | **Częściowo po T18:** admin + import walidują `effect_json`, a runtime consumabli używa już `effect_json` / fallback legacy; pełne usunięcie płaskich `effect_*` nadal czeka na migrację treści |
