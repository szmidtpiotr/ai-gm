...
// ── Cell tooltip singleton ────────────────────────────────────────────────────
function _getTooltip() {
  let tip = document.getElementById("__admin_cell_tip__");
  if (!tip) {
    tip = document.createElement("div");
    tip.id = "__admin_cell_tip__";
    tip.className = "admin-cell-tooltip";
    document.body.appendChild(tip);
  }
  return tip;
}
function _showCellTooltip(anchor, text) {
  ...
  requestAnimationFrame(() => {
    ...
    tip.style.top  = (above >= 4 ? above : rect.bottom + 6) + "px";
    tip.style.left = Math.max(4, Math.min(rect.left, window.innerWidth - maxW - 10)) + "px";
  });
}
function _hideCellTooltip() {
  const tip = document.getElementById("__admin_cell_tip__");
...
export function renderTable(container, columns, rows, options = {}) {
  if (!columns) {
    container.innerHTML = '<p class="section-note" style="color:var(--text-muted)">Ładowanie…</p>';
    return;
  }

  // ── Column width persistence ──
  const tableId    = options.tableId || null;
  const COL_W_KEY  = tableId ? `aigm_col_w_${tableId}` : null;
  let savedColW    = {};
  if (COL_W_KEY) { try { savedColW = JSON.parse(localStorage.getItem(COL_W_KEY) || "{}"); } catch {} }
  const colWidths  = { ...savedColW };
  ...
  columns.forEach(col => {
    ...
    if (colWidths[col.key]) c.style.width = colWidths[col.key] + "px";
    colgroup.appendChild(c);
    colEls.push(c);
  });
  if (onDelete || extraActions) {
    const c = document.createElement("col"); c.style.width = "120px"; colgroup.appendChild(c);
  }
  ...
  columns.forEach((c, cIdx) => {
    ...
    rh.addEventListener("mousedown", ev => {
      ...
      const colEl  = colEls[cIdx];
      const startW = colEl?.offsetWidth || th.offsetWidth || 100;
      ...
      function onUp() {
        ...
        if (COL_W_KEY) { try { localStorage.setItem(COL_W_KEY, JSON.stringify(colWidths)); } catch {} }
      }
  ...
  if (rows === null) {
    ...
    displayRows.forEach((row) => {
      ...
      if (typeof options.rowClass === "function") {
        const cls = (options.rowClass(row) || "").trim();
        if (cls) cls.split(/\s+/).forEach((c) => tr.classList.add(c));
      }
      ...
      columns.forEach((col, colIdx) => {
        ...
        if (col.editable && onEdit && col.type === "badge" && col.editType === "select" && Array.isArray(col.editOptions)) {
          ...
          const fullVal = String(row[col.key] ?? "");
          const shown = typeof col.formatDisplay === "function"
            ? String(col.formatDisplay(row))
            : (col.popup && fullVal.length > 48 ? fullVal.substring(0, 48) + "…" : textVal);
          ...
          if (col.popup && fullVal.trim()) {
            span.classList.add("has-cell-popup");
            span.addEventListener("mouseenter", () => _showCellTooltip(span, fullVal));
            span.addEventListener("mouseleave", _hideCellTooltip);
          }
          if (col.tooltipField) {
          ...
          if (col.type === "badge") {
            ...
            pill.textContent = typeof col.formatDisplay === "function"
              ...
              : String(row[col.key] ?? "");
            td.appendChild(pill);
          } else if (typeof col.formatDisplay === "function") {
