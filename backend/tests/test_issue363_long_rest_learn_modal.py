...
# ─── Test 5: Frontend app.js używa stat_value_ceiling z API ──────────────────

def test_frontend_uses_api_stat_ceiling():
    """app.js NIE może mieć hardkodowanego '>20' w logice statystyk."""
    appjs_paths = [
        "/app/frontend/front/js/app.js",
        os.path.join(os.path.dirname(__file__), "../../frontend/front/js/app.js"),
    ]
    content = None
    for path in appjs_paths:
        try:
            with open(path) as f:
                content = f.read()
            break
    ...
    if content is None:
        pytest.skip("app.js nie znaleziony w kontenerze — test tylko lokalnie")

    assert "newVal > 20" not in content, (
        "app.js używa 'newVal > 20' jako hardkodowanego sufitu statystyki. "
        "Po C8 sufit to 19 — kod musi używać stat_value_ceiling z API."
    )


# ─── Test 6: Frontend app.js używa skill_rank_ceiling z API ──────────────────

def test_frontend_uses_api_skill_ceiling():
    """app.js NIE może mieć hardkodowanego 'rank < 5' dla umiejętności."""
    appjs_paths = [
        "/app/frontend/front/js/app.js",
        os.path.join(os.path.dirname(__file__), "../../frontend/front/js/app.js"),
    ]
    ...
    assert "rank < 5" not in content, (
        "app.js używa 'rank < 5' jako hardkodowanego sufitu rangi umiejętności. "
        "Po C7 sufit to 3 — kod musi używać skill_rank_ceiling z API."
    )


# ─── Test 7: Frontend ma modal odpoczynku z opcją Ucz się ────────────────────

def test_frontend_has_rest_modal_with_learn_option():
    """index.html lub app.js powinny zawierać przycisk/element 'Ucz się' w kontekście odpoczynku."""
    files_to_check = [
    ...
    for path in files_to_check:
        try:
            with open(path) as f:
                content = f.read()
            if "Ucz się" in content or "ucz-sie" in content or "rest-learn" in content or "learn-rest" in content:
                found = True
                break
        except FileNotFoundError:
            continue

    if all(not os.path.exists(p) for p in files_to_check):
        pytest.skip("Frontend files not found in test environment")

    assert found, (
        "Brak elementu 'Ucz się' w modal odpoczynku (index.html lub app.js). "
        "C9 wymaga: kliknięcie Długi odpoczynek → modal z opcjami 'Ucz się' / 'Śpij'."
    )
