"""Seed L17 — game_dungeons record 'ruiny_probna' + world_hexes dungeon entry.

Run inside dev backend container:
  docker exec ai-gm-dev-backend-1 python3 /app/scripts/seed_l16_ruiny_probna.py
...
DUNGEON = {
    "key": "ruiny_probna",
    "label": "Ruiny Twierdzy",
    "location_key": "ruiny_twierdzy",
    "rooms": 6,
    "enemy_pool": json.dumps([
        "bandit", "bandit_thug", "brigand", "orc", "orc_warrior",
        "orc_shaman", "mercenary", "thug", "guard", "troll",
        "ogre", "giant_rat", "giant_spider", "wolf", "kobold", "cultist",
    ]),
    "boss_enemy": "bandit_chief",
    "loot_tier": "rare",
    "atmosphere": (
        "Osmalone mury dawnej fortecy, rozdarte wyłomami po katapultach. "
        "Spod gruzu sterczą zwęglone belki, a powietrze wciąż gryzie swądem "
        "spalenizny i zimnego popiołu. Po niedawnym oblężeniu twierdzę przejęli "
        "zbóje i najemnicy — ich herszt rozsiadł się w spalonej sali tronowej, "
        "pośród grabionych łupów i połamanej broni."
    ),
    "cooldown_hours": 48,
    "min_level": 2,
    "is_active": 1,
    ...
    "tile_category_key": "ruiny_twierdzy",
    "tile_count": 6,
    "boss_tile_id": 206,   # Sala Zbójeckiego Króla (is_boss_tile=1, 4-door)
    "endless_growth_n": 0,
}
    ...
    if existing:
        ...
        conn.execute(
            """INSERT INTO game_dungeons
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (
                DUNGEON["key"], DUNGEON["label"], DUNGEON["location_key"],
                DUNGEON["rooms"], DUNGEON["enemy_pool"], DUNGEON["boss_enemy"],
                DUNGEON["loot_tier"], DUNGEON["atmosphere"], DUNGEON["cooldown_hours"],
                DUNGEON["min_level"], DUNGEON["is_active"], DUNGEON["chest_loot_table_key"],
                DUNGEON["boss_loot_table_key"], DUNGEON["room_loot_chance"],
                DUNGEON["room_types_json"], DUNGEON["riddle_source"],
                DUNGEON["riddle_max_hints"], DUNGEON["dungeon_difficulty"],
                DUNGEON["tile_category_key"], DUNGEON["tile_count"],
                DUNGEON["boss_tile_id"], DUNGEON["endless_growth_n"],
            ),
        )
        conn.commit()
        print(f"[OK] Dungeon '{DUNGEON['key']}' created (D2, ruiny_twierdzy, boss_tile_id=206)")
    ...
    if existing_hex:
        ...
        conn.execute(
            """INSERT INTO world_hexes
               ...
               VALUES (?, ?, 'dungeon', ?, ?, ?, 1)""",
            (HEX_Q, HEX_R, DUNGEON["label"], DUNGEON["atmosphere"][:200], DUNGEON["location_key"]),
        )
        conn.commit()
