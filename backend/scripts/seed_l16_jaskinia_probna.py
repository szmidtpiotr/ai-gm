"""Seed L16 — game_dungeons record 'jaskinia_probna' + world_hexes dungeon entry.

Run inside dev backend container:
  docker exec ai-gm-dev-backend-1 python3 /app/scripts/seed_l16_jaskinia_probna.py
...
DUNGEON = {
    "key": "jaskinia_probna",
    "label": "Jaskinia Próbna",
    "location_key": "jaskinia_gory_mgly",
    "rooms": 6,
    "enemy_pool": json.dumps([
        "cave_bear", "wolf", "giant_spider", "kobold", "goblin",
        "giant_rat", "troll", "slime", "harpy",
    ]),
    "boss_enemy": "ancient_troll",
    "loot_tier": "common",
    "atmosphere": (
        "Wilgotne groty głęboko w Górach Mgły. Stalaktyty kapią lodowatą wodą, "
        "gdzieś w mroku słychać przytłumione echa kroków i gromkie buczenie. "
        "Ściany pokrywa świecąca grzybnia i mech; powietrze pachnie ziemią i zgnilizną. "
        "Idealne miejsce by przetestować refleks i odwagę przed wejściem w głębsze ostępy."
    ),
    "cooldown_hours": 48,
    "min_level": 2,
    "is_active": 1,
    "chest_loot_table_key": "loot_common",
    ...
    "tile_count": 6,
    "boss_tile_id": 49,   # Legowisko Pradawnego Trolla (is_boss_tile=1, 4-door)
    "endless_growth_n": 0,
}

# Free dungeon hex on world map (no existing location_key)
HEX_Q = -48
HEX_R = -16
...
def seed(conn: sqlite3.Connection) -> None:
    ...
    if existing:
        ...
        print(
            ...
            f"(D2, jaskinie, tile_count=6, boss_tile_id=49)"
        )
