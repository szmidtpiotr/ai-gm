"""Seed L17 — game_dungeons record 'zamek_probna' + world_hexes dungeon entry.

Run inside dev backend container:
  docker exec ai-gm-dev-backend-1 python3 /app/scripts/seed_l16_zamek_probna.py
...
DUNGEON = {
    "key": "zamek_probna",
    "label": "Nawiedzony Zamek",
    "location_key": "nawiedzony_zamek",
    "rooms": 6,
    "enemy_pool": json.dumps([
        "skeleton", "zombie", "ghoul", "ghost", "wraith",
        "shadow_stalker", "dark_mage", "dark_priest", "cultist",
        "vampire", "giant_spider", "giant_rat",
    ]),
    "boss_enemy": "lich",
    "loot_tier": "rare",
    "atmosphere": (
        "Omszałe mury starego zamku, w którym dawno zgasło życie. Zielonkawa "
        "nekromancka mgła pełznie nisko po popękanych płytach, a w mroku szepczą "
        "głosy umarłych. Wśród sarkofagów, kości i świecących run włada lisz — "
        "pan tego nawiedzonego dworu, strzegący sekretów zza grobu."
    ),
    "cooldown_hours": 48,
    "min_level": 3,
    "is_active": 1,
    ...
    "tile_category_key": "nawiedzony_zamek",
    "tile_count": 6,
    "boss_tile_id": 265,   # Komnata Lisza (is_boss_tile=1, 4-door)
    "endless_growth_n": 0,
}
...
def seed(conn: sqlite3.Connection) -> None:
    ...
    if existing:
        ...
        conn.commit()
        print(f"[OK] Dungeon '{DUNGEON['key']}' created (D3, nawiedzony_zamek, boss_tile_id=265)")
