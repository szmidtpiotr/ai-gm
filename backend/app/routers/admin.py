...
class WeaponCreateReq(BaseModel):
    ...
    finesse: bool = False
    light: bool | None = None  # #1397: jawna lekkość (oburęczność); None → heurystyka finesse
...
def admin_create_enemy(req: EnemyCreateReq, _: None = Depends(require_admin_token)):
    try:
        ...
        if str(e) == "invalid_stats_json":
            raise HTTPException(status_code=422, detail="stats_json must be an object: {STR|DEX|CON|INT|WIS|CHA|LCK: integer}") from None
...
def admin_overview(_: None = Depends(require_admin_token)):
    ...
    try:
        ...
        llm_preset_name = (
            llm_snap.get("active_preset_label")
            or (llm_snap.get("settings") or {}).get("model")
            or "—"
        )
