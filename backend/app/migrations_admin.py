...
ADMIN_SEEDS = [
    ...
    VALUES
    ('poisoned', 'Poisoned', '{"schema_version":1,"effect_category":"character_condition","effects":[{"type":"static_stat_modifier","stat":"STR","value":-2,"expires":"duration_rounds:3"},{"type":"dot","value":"1d4","damage_type":"poison","tick":"start_turn","expires":"duration_rounds:3"}]}', 'Zatruty — -2 SIŁY oraz 1k4 obrażeń od trucizny na początku każdej tury przez 3 rundy. (DoT wartość startowa, Sandbox-tunable.)', 1, NULL, datetime('now'), datetime('now'))
    """,
    ...
    ('frozen', 'Zmrożony', '{"schema_version":1,"effect_category":"character_condition","effects":[{"type":"static_stat_modifier","stat":"DEX","value":-4},{"type":"block_action"},{"type":"periodic_save","stat":"CON","value":14,"tick":"start_turn","expires":"save_success"}]}', 'Postać skuta lodem — odbiera akcje (nie może działać) i DEX -4. Udany rzut CON DC 14 (rozgrzewanie) na początku tury lub kontakt z ciepłem wyrywa z lodu; póki lód trzyma, tura przepada.', 1, 0, NULL, NULL, datetime('now'), datetime('now')),
...
def _ensure_character_race_column(conn: sqlite3.Connection) -> None:
    """#970 R1 — pole `race` (TEXT DEFAULT 'human') w tabeli characters."""
    try:
        conn.execute("ALTER TABLE characters ADD COLUMN race TEXT NOT NULL DEFAULT 'human'")
        conn.commit()
    except Exception as e:
